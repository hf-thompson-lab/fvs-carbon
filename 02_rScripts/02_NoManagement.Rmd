...
RMSE for Standing Carbon by Condition is \~15, which is close to that
for Standing Carbon by Plot, but Standing Carbon by Plot has the lowest
RMSE. Therefore we will use Standing Carbon by Plot for the remainder of
this analysis.
...
The FIA COND table lists when inventories were performed and
observations from those inventories. We can use the above established
connection between FVS identifiers and FIA COND identifiers to find
subsequent surveys for the conditions included.
...
# - the first year inventoried, earliest first; then
# - break ties by the second year inventoried, earliest year first; then
...
skipped <- matching_cond |>
  ...
  select(STATECD, UNITCD, COUNTYCD, PLOT, INVYR, COND_STATUS_CD)
knitr::kable(skipped)
```
...
```{r}
nk_to_fia <- read_csv("../03_outputs/nk_to_fia.csv",
    col_types = cols(
      `FIA plot code` = col_character(),
      .default = col_number()
    )
...
nk_to_fvs <- read_csv("../03_outputs/nk_to_fvs.csv",
    col_types = cols(
      `FIA plot code` = col_character(),
...
grow_only <- matching_cond |>
  ...
  left_join(numbered_plots, by = join_by(STATECD, UNITCD, COUNTYCD, PLOT)) |>
  left_join(nk_to_fia, by = join_by(STATECD, UNITCD, COUNTYCD, PLOT)) |>
  left_join(nk_to_fvs, by = join_by(`FIA plot code`))
...
x <- matching_cond |>
  ...
  left_join(
    nk_to_fia |>
      select(STATECD, UNITCD, COUNTYCD, PLOT, `FIA plot code`),
    by = join_by(STATECD, UNITCD, COUNTYCD, PLOT)
  ) |>
  left_join(
    nk_to_fvs |>
      select(`FIA plot code`, FVS_STAND_ID),
    by = join_by(`FIA plot code`)
  ) |>
  arrange(INVYR, FVS_STAND_ID)
