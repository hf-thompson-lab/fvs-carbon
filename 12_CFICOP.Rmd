...
After talking with folks, we're going to harvest by basal area by
diameter class by species by plot by time.
...
tmp_observed <- cfiabp_trees |>
  ...
  filter(USFSCode == 129) |>
  filter(VisitTreeStatusCode == 1) |>
  filter(1990 <= VisitCycle & VisitCycle <= 2000) |>
  mutate(
    Year = VisitCycle,
...
cficop_none_carbon |>
  ...
  mutate(
    name = case_match(
      ...
      "Adjusted" ~ "Best Effort",
      .default = name
    )
  ) |>
  mutate(name = ordered(
    name,
    levels = c("Standard", "Calibrated", "Best Effort", "Reference", "Measured")
  )) |>
