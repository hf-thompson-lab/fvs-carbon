...
```{r plot cfi_harvested, eval=FALSE}
qryDWSPCFIPlotVisitTreeDetail |>
  cfi_with_visit_info(tblDWSPCFIPlotVisitsComplete) |>
...
```{r cfi disturbance analysis, eval=FALSE}
tblDWSPCFIPlotVisitsComplete |>
  cfi_stable_location() |>
  cfi_quabbin() |>
  distinct(VisitCycle, MasterPlotVisitID, MasterPlotID, VisitYear) |>
  rename(PlotVisitID = MasterPlotVisitID) |>
  left_join(tblDWSPCFIPlotVisitDisturbances, by = join_by(PlotVisitID)) |>
  mutate(
    Disturbed = 
      # > 0 means disturbed
      ...
      # if severity is provided, only accept severity > 1
  ...
  group_by(MasterPlotID) |>
  summarize(Disturbed = any(Disturbed, na.rm = TRUE)) |>
  ungroup() |>
...
```{r}
trees283_6_defpropasp <- trees283_6_defpropasp |>
  cfi_disturbed()
trees283_6_defpropasp |>
  group_by(MasterPlotID) |>
  summarize(PlotDisturbed = any(PlotDisturbed, na.rm = TRUE)) |>
  ungroup() |>
  ggplot(aes(PlotDisturbed)) +
  geom_bar()
```
...
trees283_6_defpropasp |>
  group_by(MasterPlotID) |>
  summarize(
    PlotHarvested = any(PlotHarvested, na.rm = TRUE),
    PlotDisturbed = any(PlotDisturbed, na.rm = TRUE)
  ) |>
  ungroup() |>
  mutate(
    Status = if_else(PlotHarvested & PlotDisturbed, "Harvested & Disturbed",
                     if_else(PlotHarvested, "Harvested",
                             if_else(PlotDisturbed, "Disturbed", "Neither"))),
    Status = ordered(Status, levels = c("Harvested & Disturbed", "Disturbed", "Harvested", "Neither"))
  ) |>
  ggplot(aes(Status)) +
  geom_bar() +
  geom_text(aes(y = after_stat(count + 5), label = after_stat(count)), stat = "count") +
...
So, Severity has only really been recorded since 2010.

What if we look for any disturbance at all?

```{r}
plt_vst_rhs |>
  filter(PlotVisitDisturbanceYear > VisitYear - 10 & PlotVisitDisturbanceYear < 9999) |>
  filter(is.na(PlotVisitDisturbanceSeverity) | PlotVisitDisturbanceSeverity > 1) |>
  ggplot(aes(PlotVisitDisturbanceYear, fill = DisturbanceDesc, group = DisturbanceDesc)) +
  geom_bar(position = "stack") +
  scale_fill_discrete(name = "Disturbance Type") + 
  theme_bw() +
  ggtitle("Disturbance prevalance in CFI Quabbin plots") +
...
plt_vst_rhs |>
  mutate(
    Disturbed = (PlotVisitDisturbanceCode > 0) &
      (is.na(PlotVisitDisturbanceSeverity) | (PlotVisitDisturbanceSeverity > 1))
  ) |>
  group_by(MasterPlotID) |>
  summarize(Disturbed = any(Disturbed, na.rm = TRUE)) |>
