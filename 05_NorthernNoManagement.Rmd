...
For now, we use the simple method of imputing the most common non-spp.
species of the genus on the plot for all spp. trees.
...
Of note, FIA seems to not note species of Hawthorn or Pear in NE:
```{r unfixed spcd}
tree_growth_with_fixed_spcd |>
  filter(SPECIES == "spp.") |>
  group_by(SPCD) |>
  ...
  ungroup() |>
  left_join(species_mixin, by = join_by(SPCD)) |>
  select(SCIENTIFIC_NAME, COMMON_NAME, TREE_COUNT)
```
...
ingrowth_stem_rate <- tree_growth_with_fixed_spcd |>
  ...
  # If PREV_MEASYEAR isn't filled in, estimate it as 5 years prior.
  mutate(PREV_MEASYEAR = if_else(is.na(PREV_MEASYEAR), MEASYEAR - 5, PREV_MEASYEAR)) |>
  group_by(STATECD, COUNTYCD, PLOT, SPCD) |>
  summarize(
    FIRST_YEAR = min(PREV_MEASYEAR),
    LAST_YEAR = max(MEASYEAR),
    COUNT = n(),
    COUNT_PER_ACRE = COUNT * 6, # Don't use TPA_UNADJ; microplot multipliers are inaccurate here.
    .groups = "keep"
  ) |>
  ungroup() |>
  left_join(species_mixin, by = join_by(SPCD)) |>
