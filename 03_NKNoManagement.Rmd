...
tar_load(nk_to_fia)
files <- nk_project_grow_only("data/fvs", fiadb, nk_to_fia, nk_regen)
keywordfile <- files[1]
outputdb <- files[2]
```
...
fvs <- processx::process$new(
  "/fvs/fvsbin/fvsne.exe",
  paste0("--keywordfile=", basename(keywordfile)),
  wd = dirname(keywordfile)
)
fvs$wait()
print(fvs$get_exit_status())
```
