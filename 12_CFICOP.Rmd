...
cfigro_bai_task$select(
  c("Acer_rubrum", "LAT", "Projected_BA", "Projected_Tph", "Pinus_strobus")
)
...
tmp_plot_starting_ba <- cficop_none_summary |>
  filter(Year == 1970) |>
  mutate(Projected_BA = conv_multiunit(BA, "ft2 / acre", "m2 / hectare")) |>
  group_by(StandID) |>
  summarize(
    Starting_BA = sum(Projected_BA),
    .groups = "drop"
  )

tmp_plot_starting_carbon <- cficop_none_carbon |>
  filter(Year == 1970) |>
  group_by(StandID) |>
  summarize(
    Starting_Carbon = sum(Aboveground_Total_Live),
    .groups = "drop"
  )

tmp_plot_projected_carbon <- cficop_none_carbon |>
  filter(Year == 2020) |>
  group_by(StandID) |>
  summarize(
    Projected_Carbon = sum(Aboveground_Total_Live),
    .groups = "drop"
  )

cficop_none_bai <- cficop_none_summary |>
  filter(Year == 2020) |>
  mutate(
    Projected_BA = conv_multiunit(BA, "ft2 / acre", "m2 / hectare"),
    Projected_Tph = conv_unit(Tpa, "hectare", "acre")
  ) |>
  select(StandID, Projected_BA, Projected_Tph) |>
  left_join(
    cfigro_plot |>
