...
tar_target(
  nrs_harvest_prsc,
  {
    ...
    harvest_for_fvs <- fia_trees_filtered(
        ...
        filter = \(.data, com) {
          .data |> select(CN, STATECD, COUNTYCD, PLOT, SUBP, TREE)
      }) |>
      inner_join(
    ...
    stopifnot(harvest_for_fvs |> filter(CN != PREV_TRE_CN) |> nrow() == 0)
...
keyfile_thin_section <- function(StandCN) {
  # StandCN refers to STAND_CN in stand_stats
