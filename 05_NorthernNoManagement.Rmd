...
rm(fia, plt_cns)
print(nrow(ingrowth))
print(nrow(ingrowth |> filter(DIA_END > 3)))
```
...
```{r dbh_gt_3}
starting_trees |> filter(DIA > 3) |> nrow()
```
...
For the remaining ~40,000 trees, we will need to back-project.
...
if (file.exists(rds_file)) {
  ...
  tree_negative_growth <- tbl(fia, 'TREE') |>
    semi_join(
    ...
    left_join(next_tre_mixin, by = join_by(CN)) |>
    # Filter to only the most recent trees (which have no next tree)
    filter(is.na(NEXT_TRE_CN)) |>
    mutate(FRST_TRE_CN = CN, FRST_DIA = DIA) |>
    select(CN, DIA, PREV_TRE_CN, FRST_TRE_CN, FRST_DIA) |>
    collect()
  
  prev_tre_cns <- tree_negative_growth |>
    select(PREV_TRE_CN) |>
    filter(!is.na(PREV_TRE_CN)) |>
    distinct()
  ...
  while (nrow(prev_tre_cns) > 0) {
    ...
    tmp_trees <- tbl(fia, 'TREE') |>
      # Process chunks of 1000 trees, since SQLite has a hard time
      # doing the semi join on more records than that
      semi_join(prev_tre_cns |> head(1000), by = join_by(CN == PREV_TRE_CN), copy = TRUE) |>
      rename(PREV_DIA = DIA) |>
      select(CN, PREV_DIA, PREV_TRE_CN) |>
      collect()
    tree_negative_growth <- tree_negative_growth |>
  ...
  dbDisconnect(fia)  
  rm(next_tre_mixin, prev_tre_cns, tmp_trees, iterations)
...
fia <- DBI::dbConnect(RSQLite::SQLite(), 'data/raw/SQLite_FIADB_ENTIRE.db')
species_mixin <- tbl(fia, 'REF_SPECIES') |>
  select(SPCD, E_SPGRPCD, SCIENTIFIC_NAME) |>
  left_join(
...
projected_annual_growth <- starting_trees |>
  ...
  mutate(ANN_DIA_PROJ = conv_unit(ANN_DIA_PROJ, "in", "cm")) |>
  mutate(ANN_DIA_ERR = DIA_ERR / (MEASYEAR - FRST_MEASYEAR)) |>
  mutate(ANN_DIA_ERR = conv_unit(ANN_DIA_ERR, "in", "cm")) |>
...
annual_vs_decadal <- stand_stats |>
  ...
  mutate(
    ...
    Starting_Carbon = max(if_else(Year == First_Year, Decadal_Carbon, 0), na.rm = TRUE),
    Decadal_Carbon_Delta = Decadal_Carbon - Starting_Carbon,
    Decadal_Carbon_Flux = if_else(Year == First_Year, 0, Decadal_Carbon_Delta / (Year - First_Year)),
    Annual_Carbon_Delta = Annual_Carbon - Starting_Carbon,
...
ggplot(
    annual_vs_decadal |> filter(Projection_Years > 0),
