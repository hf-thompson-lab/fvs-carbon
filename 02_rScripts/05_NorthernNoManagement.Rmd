...
modern_plots_filter <- function(.data) {
  ...
  # but continued to be used after. This will filter to
  # plots that have surveys that use the national plot
  # design, but some plots use the same id with multiple
  # designs, so post-filtering is also required.
...
ggplot(
  ...
  facet_wrap(~`Forest Type Group`) +
  ggtitle("Grow-Only Stands\nChange in Measured BA (m² / ha) vs. Projection Years")
```
...
projected_vs_measured <- stand_stats |>
  ...
  mutate(
    ...
    Min_Measured_Carbon_Delta = min(Measured_Carbon_Delta), # Might be < 0
  ...
  #filter(Min_Measured_Carbon_Delta < 0) |>
...
ggplot(
  ...
  facet_wrap(~`Forest Type Group`) +
  ggtitle("Grow-Only Stands\nMeasured vs. Projected Carbon Change (Mg/ha)") +
...
ggplot(
  ...
  geom_abline(intercept = 0, slope = 1) +
  facet_wrap(~`Forest Type Group`) +
  ggtitle("Grow-Only Stands\nMeasured vs. Projected Basal Area (m²/ha)")# +
  #coord_cartesian(xlim = c(0, 300), ylim = c(0, 300))
```
