...
[@Herbert2023] use FIA measurement of biomass to determine carbon. The data
used is from table GFND_LYR_FNCTL_GRP; FIA notes that "Currently, this table is
populated only by the PNWRS FIA work unit (SURVEY. RSCD = 27)."

We can instead use the FVS Fire and Fuels extension (FFE-FVS), which contains
a Carbon Submodel, to compute
carbon directly from FIA data. This has the added benefit of ensuring consistent
calculation for measured and projected carbon.
...
FIA has carbon in the tree table, in CARBON_AG; we can use this instead
of depending on FVS's carbon calculation.
...
* scale the carbon in each trees for the area we're using to get carbon per acre
...
useful_subp_cond <- matching_subp_cond |>
  ...
  filter(SUBP <= 4) |> # FIA introduces estimation plots with ID > 4; ignore those
...
#       Example: a full plot is 672.4535 ha; filter out anything below
#       89.2% of that
