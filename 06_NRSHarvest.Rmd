...
```{r nrshrv_prsc_treelist, eval = FALSE}
...
```{r harvested trees cut, eval = FALSE}
cut_trees <- nrshrv_prsc_cutlist |>
  select(StandID, PtIndex, TreeId) |>
  rename(
    STAND_ID = StandID,
    SUBP = PtIndex,
    TREE = TreeId
...
harvested_and_cut <- nrshrv_tree_harvested |>
...
fvs_read_output(nrshrv_srvy, "FVS_TreeList_East")
```
