...
fvs_spcd_mixin <- read_csv(
  ...
  select(`FIA Code`, `Species Code`) |>
  rename(SPCD = `FIA Code`, FVS_SPCD = `Species Code`)
...
BAI - Basal Area Increment - m^2 x ha^-1 x year^-1
...
ingrowth_stem_rate <- tree_growth_with_fixed_spcd |>
  ...
  group_by(STATECD, COUNTYCD, PLOT) |>
  mutate(
    # Ingrowth is often observed in the first inventory, for which we have no
    # previous measurement year. Create a synthetic measurement year 5 years
    # prior to the first measurement to accommodate this.
    FIRST_YEAR = min(MEASYEAR) - 5,
...
ingrowth_stem_rate |>
  ggplot(aes(y = RATE_PER_ACRE, fill = fct_rev(reorder(GROUP_NAME, NUM_PLOTS)))) +
  geom_boxplot(outlier.size = 0.1) +
  scale_fill_discrete(
    name = "Species Group"
  ) +
...
We pick a few trees closest to 3" DBH, do a linear interpolation of their
height to what it might be at 3" DBH, and average them.

```{r tree_growth_ht}
tree_growth_ht_plot <- tree_growth_with_fixed_spcd |>
...
tree_growth_ht_state <- tree_growth_with_fixed_spcd |>
  mutate(HT = coalesce(HT, PREV_HT), DIA = coalesce(DIA, PREV_DIA)) |>
  filter(!is.na(HT) & !is.na(DIA)) |>
  group_by(STATECD, SPCD) |>
  arrange(abs(DIA - 3)) |>
  filter(row_number() <= 9) |>
  mutate(HT_STATE = HT * 3 / DIA) |>
  filter(n() >= 3) |>
  summarize(HT_STATE = mean(HT_STATE), .groups = "keep") |>
  ungroup()

tree_growth_ht_ne <- tree_growth_with_fixed_spcd |>
  mutate(HT = coalesce(HT, PREV_HT), DIA = coalesce(DIA, PREV_DIA)) |>
  filter(!is.na(HT) & !is.na(DIA)) |>
  group_by(SPCD) |>
  arrange(abs(DIA - 3)) |>
...
tree_growth_ht_min <- tree_growth_with_fixed_spcd |>
  mutate(HT = coalesce(HT, PREV_HT), DIA = coalesce(DIA, PREV_DIA)) |>
  filter(!is.na(HT) & !is.na(DIA)) |>
  select(SPCD, DIA, HT) |>
  group_by(SPCD) |>
...
tree_growth_ht <- tree_growth_with_fixed_spcd |>
  semi_join(grm_ingrown, by = join_by(STATECD, COUNTYCD, PLOT, SUBP, TREE)) |>
  distinct(STATECD, COUNTYCD, PLOT, SPCD) |>
  left_join(tree_growth_ht_plot, by = join_by(STATECD, COUNTYCD, PLOT, SPCD)) |>
  left_join(tree_growth_ht_state, by = join_by(STATECD, SPCD)) |>
  left_join(tree_growth_ht_ne, by = join_by(SPCD)) |>
  left_join(tree_growth_ht_min, by = join_by(SPCD)) |>
  mutate(HT = coalesce(HT_PLOT, HT_STATE, HT_NE, HT_MIN))
```
...
ingrowth_by_stem <- ingrowth_stem_rate |>
 ...
 ingrowth_by_stem |>
  ...
  ggtitle("Tree Height for FVS Ingrowth")
```
...
- NATURAL 0 SPECIES TPA [no age] [no %] HEIGHT
- MERCHPREP 0
- BURNPREP 0
- TALLYONE 9
- END
...
keywordfile_section <- function(
  ...
  c(
    ...
    keyfile_regen_section(StandID, Timestep),
    fvs_kwd0("Process"),
    recursive = TRUE
  )
}
...
for (timestep in c(0, 1, 10)) {
  for (rannseed in c(0, 1, 3, 5)) {
    for (partition in 1:8) {
      for (regen in c("", "_noregen")) {
      mgmtid <- paste0(timestep, "Y", rannseed)
      filename <- paste0("data/fvs/FVS_", title, "_", mgmtid, "_", partition, regen, ".db")
...
FVS_Carbon_REGEN <- FVS_Carbon |>
  filter(Timestep == 10) |>
  filter(Regen == "_noregen") |>
#  filter(RanNSeed == 1)
  group_by(StandID, Year) |>
  summarize(
    Aboveground_Total_Live = mean(Aboveground_Total_Live),
...
ggplot(
  ...
  ylab(bquote("1-Year Timestep Annual Carbon Flux" ~ (`Mg C` %*% ha^-1 %*% year^-1))) +
  xlab(bquote("10-Year Timestep Annual Carbon Flux" ~ (`Mg C` %*% ha^-1 %*% year^-1)))
```
...
### Regeneration vs. None

Look at carbon projections with vs. without regeneration

```{r regen_vs_none}
tmp_regen <- FVS_Carbon_NONE |>
  select(StandID, Year, Total_Stand_Carbon) |>
  rename(Carbon_With_Regen = Total_Stand_Carbon)

tmp_noregen <- FVS_Carbon_REGEN |>
  select(StandID, Year, Total_Stand_Carbon) |>
  rename(Carbon_Without_Regen = Total_Stand_Carbon)

regen_vs_noregen <- tmp_regen |>
  left_join(tmp_noregen, by = join_by(StandID, Year)) |>
  group_by(StandID) |>
  filter(Year == max(Year)) |>
  ungroup()

regen_vs_noregen |>
  ggplot(aes(Carbon_Without_Regen, Carbon_With_Regen)) +
  ...
  ggtitle("FVS Projected Carbon with vs. without regeneration") +
  xlab(bquote("Carbon Without Regeneration" ~ (`Mg C` %*% ha^-1))) +
  ylab(bquote("Carbon With Regeneration" ~ (`Mg C` %*% ha^-1)))

```
```{r}
regen_vs_noregen |>
  mutate(Ratio = Carbon_With_Regen / Carbon_Without_Regen) |>
  ggplot(aes(x = Ratio)) +
  geom_histogram() +
  scale_x_continuous(transform = "log10") +
  scale_y_continuous(transform = "log10") +
  theme_bw() +
  ggtitle("Ratio of FVS Projected Carbon with vs. without regeneration") +
  ylab("Number of Plots") +
  xlab("Ratio of Projected Carbon With to Without Regeneration")
```
...
ggplot(
  ...
  scale_fill_discrete(name = "Forest Type Group") +
  theme_bw() +
  ggtitle("Measured QMD vs. Projected BA for Grow-Only Stands") +
  ylab(bquote("BA Residual " (m^2 %*% hectare^-1 %*% year^-1))) +
  xlab(bquote("Measured Stand QMD " (m)))
...
ggplot(
  ...
  theme_bw() +
  #facet_wrap(~`Forest Type Group`) +
  ggtitle("Grow-Only Stands\nProjected vs. Measured Tpa (trees / acre)")
```
...
ggplot(
  ...
  scale_fill_discrete(name = "Forest Type Group") +
  theme_bw() +
  ggtitle("Measured Tpa vs. Residula in Projected BA for Grow-Only Stands") +
  ylab(bquote("BA Residual " (m^2 %*% hectare^-1 %*% year^-1))) +
  xlab(bquote("Measured Stand Tpa " (trees / acre)))
