...
Quartiles here are also very close to 100, 125, 150.
...
harvest_summary |> ggplot(aes(StartingBA, HarvestFraction)) +
  geom_point()
```
...
It looks like using StartingBA breakpoints of c(400, 500, 600) and
HarvestFraction breakpoints of c(0.1, 0.2, 0.3) would give us a reasonable 4x4
breakdown.
...
harvest_summary <- harvest_summary |>
  mutate(
    StandBA = ordered(case_when(
      StartingBA < 400 ~ "< 400",
      StartingBA < 500 ~ "400 - 500",
      StartingBA < 600 ~ "500 - 600",
      .default = ">= 600"
    ), levels = c("< 400", "400 - 500", "500 - 600", ">= 600")),
...
```{r harvest intensity distribution}
harvest_summary |>
...
```{r how to handle time, eval = FALSE}
nrshrv_prsc_carbon |>
  group_by(StandID) |>
  arrange(Year) |>
