...
library(maps)
library(ggspatial)
library(ggpattern) # Patterns in ggplot2 charts
library(patchwork) # arrange multiple ggplot2 plots on a page
...
sample_plots <- nrshrv_plot |> slice_sample(n = 10)
```

## FIA Regen

We want actual regeneration. See what we can get from rFIA.

NB: This takes ~1 minute per plot in the sample, so ~10 minutes for 10 plots.

```{r rfia growmort, eval = FALSE}
con <- DBI::dbConnect(RSQLite::SQLite(), fiadb)

rfia_for_state <- function(state) {
  rFIA::readFIA(
    con = con,
    schema = "main",
    states = state,
    inMemory = FALSE # Each state uses >1GB RAM; we trade time for space
  )
}
...
fia <- Map(rfia_for_state, states)

rfia_grow_mort <- do.call(rbind, pbapply(sample_plots, 1, \(sample_plot) {
  state <- fvsne_states |>
    filter(STATECD == sample_plot[["STATECD"]]) |>
    _$STATE_ABBR
  county_cd <- sample_plot[["COUNTYCD"]]
  plot_cd <- sample_plot[["PLOT"]]
...
```{r rfia grow more size class, eval=FALSE}
rfia_grow_mort |> distinct(sizeClass)
```

Note that this only gives sizeClass >= 5, so ingrowth of trees >= 5" DBH. Given
the short post-harvest time window we have, most of the "release" from harvest
will only be visible in saplings 1" < DBH < 5", so let's look at other methods.

```{r plot fia growth and mortality, eval = FALSE}
rfia_grow_mort |>
  mutate(TREE_GROUP = paste0(SCIENTIFIC_NAME, " ", if_else(sizeClass < 5, "(sapling)", "(tree)"))) |>
  left_join(nrshrv_plot |> select(CN, STATECD, COUNTYCD, PLOT), by = join_by(PLT_CN == CN)) |>
  left_join(
    nrshrv_plot_stats |>
      filter(HARVEST == 1) |>
  ...
  geom_col() +
  theme_bw() +
  ggtitle("Post-Harvest Ingrowth from rFIA")
```

## FIA GRM

We can fetch Growth / Regeneration / Mortality (GRM) data directly from the FIA
database. This includes information about saplings 1" <= DBH <= 5" on
microplots, so should be more informative about what's going on in the early
years post-harvest.

```{r fia grm regen, eval = FALSE}

# Expand sample_plots to include all inventories for the chosen sample.
sample_plots <- nrshrv_plot |>
  semi_join(sample_plots |> head(1), by = join_by(STATECD, COUNTYCD, PLOT))

# Fetch all ingrowth for all inventories on the sample plots
fia_grm <- fia_grm_ingrowth(fiadb, sample_plots) |>
  # Estimate height at the time of inventory
  mutate(HT = (DIA_END / ANN_DIA_GROWTH) * ANN_HT_GROWTH) |>
  # Get harvest year for the plot
  left_join(
    nrshrv_plot |>
...
# Fetch species information for these trees
fia_grm <- fia_grm |>
  left_join(
    fia_trees_by_cn(
      fiadb,
...
fia_grm |>
  group_by(GENUS) |>
  mutate(
    GROUP = paste0(GENUS, ", n=", n()),
    GROUP_SIZE = n()
  ) |>
  ungroup() |>
  ggplot(aes(DIA_END, HT, color = fct_reorder(GROUP, desc(GROUP_SIZE)))) +
  geom_point()
```

Much faster, and many trees with DBH < 5". Height estimation clearly has
outliers, especially among the Poplar (Liriodendron) and Cedar (Thuja).
...
fia_grm |>
  group_by(YearsSinceHarvest, GENUS, SIZE_CLASS) |>
  summarize(
    TPA = sum(TPA, na.rm = TRUE) / n(),
    .groups = "keep"
  ) |>
  ungroup() |>
  mutate(TREE_GROUP = paste0(GENUS, " (", SIZE_CLASS, ")")) |>
  ggplot(aes(YearsSinceHarvest, TPA, fill = GENUS, pattern = SIZE_CLASS)) +
...
Which gets us a bunch of saplings 1" <= DBH <= 5", so much better visibility
earlier in the growth timeline.

## Early Mortality

Seedlings and saplings die at a high rate. Let's see if we can find rates.
...
mort_rate |>
  group_by(YearsSinceHarvest, GENUS, SIZE_CLASS) |>
  summarize(TPA = sum(TPAMORT, na.rm = TRUE) / n(), .groups = "keep") |>
  ungroup() |>
  mutate(TREE_GROUP = paste0(GENUS, " (", SIZE_CLASS, ")")) |>
  ggplot(aes(YearsSinceHarvest, TPA, fill = GENUS, pattern = SIZE_CLASS)) +
  geom_col_pattern(pattern_fill = "grey20", pattern_density = 0.05) +
...
This includes all mortality, so we see tree as well as sapling mortality, but
it is dominated by sapling mortality.
...
r <- fia_grm |>
  group_by(YearsSinceHarvest, GENUS, SIZE_CLASS) |>
  summarize(TPA = sum(TPA, na.rm = TRUE) / n(), .groups = "keep") |>
  ungroup() |>
  mutate(YearsSinceHarvest = YearsSinceHarvest - (.5 + bar_width/2)) |>
  mutate(GRM = "Recruit")
...
ggplot(r, aes(YearsSinceHarvest, TPA, fill = GENUS, pattern = SIZE_CLASS, label = GRM)) +
  geom_col_pattern(width = bar_width, pattern_fill = "grey20", pattern_density = 0.05) +
  geom_col_pattern(data = m, width = bar_width, pattern_fill = "grey20", pattern_density = 0.05) +
...
seedlings |>
  ...
  ungroup() |>
  mutate(TPA = TPA / 100) |>
  mutate(YearsSinceHarvest = YearsSinceHarvest - (0.5 - bar_width/2)) |>
...
We'll put together recruitment similar to how we did for grow-only plots, but
instead of creating a recruitment rate, we'll put in groups of specific species
in specific years.

```{targets nrshrv_estab, tar_simple = TRUE}
...
```{r plot nrshrv_estab, eval = FALSE}
nrshrv_estab |>
  left_join(nrshrv_plot |> select(CN, STATECD, COUNTYCD, PLOT), by = join_by(STAND_CN == CN)) |>
  left_join(nrshrv_plot_stats |> filter(HARVEST == 1) |> select(STATECD, COUNTYCD, PLOT, MEASYEAR, HRVYR), by = join_by(STATECD, COUNTYCD, PLOT)) |>
  left_join(species_crosswalk |> filter(!is.na(FVS_SPCD)), by = join_by(SPECIES == FVS_SPCD)) |>
  mutate(YearsSinceHarvest = MEASYEAR - HRVYR) |>
  group_by(YearsSinceHarvest, GENUS, SIZE_CLASS) |>
  summarize(COUNT = mean(DENSITY), .groups = "keep") |>
  ungroup() |>
  ggplot(aes(YearsSinceHarvest, COUNT, fill = GENUS, pattern = SIZE_CLASS)) +
  geom_col_pattern()
```

...which is a mess, but seems to indicate that there are lots of saplings being
recruited.
...
FVS produces three tree tables:
 ...
 > As of FVS 2026.04, FVS_TreeList_East is merged into FVS_TreeList
 > As of FVS 2026.04, FVS_CutList_East is merged into FVS_CutList
 > As of FVS 2026.04, FVS_ATRTList_East is merged into FVS_ATRTList
...
nrshrv_prsc_treelist <- fvs_read_output(nrshrv_prsc, "FVS_TreeList") |>
  group_by(StandID) |>
  filter(
...
nrshrv_prsc_atrtlist <- fvs_read_output(nrshrv_prsc, "FVS_ATRTList") |>
  group_by(StandID) |>
  filter(
...
nrshrv_prsc_cutlist  <- fvs_read_output(nrshrv_prsc, "FVS_CutList") |>
  group_by(StandID) |>
  filter(
    Year == min(Year, na.rm = TRUE),
    random_seed == min(random_seed, na.rm = TRUE)
...
fia_harvested_ba <- fia_harvested_trees |>
  ...
  summarize(FIA_BA = sum(BA), .groups = "keep") |>
  ungroup()

fvs_harvested_ba <- fvs_harvested_trees |>
  group_by(STAND_ID, SPCD) |>
  summarize(FVS_BA = sum(BA), .groups = "keep") |>
  ungroup()
...
ggplot(align_by_ba_2, aes(PROPORTION)) +
  ...
  theme_bw() +
  ggtitle("Proportion of BA specified for harvest that was actually harvested") +
...
It's hard to see what's going on when plotting individual stands.
Let's see if we can group by harvest intensity.

```{r starting basal area, eval = FALSE}
starting_ba <- nrshrv_prsc_treelist |>
  group_by(StandID) |>
  filter(Year == min(Year)) |>
  summarize(
    StartingBA = sum(TPA * pi * (DBH / 12 / 2)^2, na.rm = TRUE),
    .groups = "keep"
  ) |>
  ungroup() |>
  mutate(
    StartingBA = conv_multiunit(StartingBA, "ft2 / acre", "m2 / hectare")
  )
starting_ba |>
  ggplot(aes(StartingBA)) +
  geom_histogram()
```

Aside: does the BA calculated from FVS trees line up with BALIVE from FIA?

```{r ba vs balive, eval = FALSE}
ba_vs_balive <- nrshrv_prsc_treelist |>
  group_by(StandID) |>
  filter(Year == min(Year)) |>
  summarize(
    StartingBA = sum(TPA * pi * (DBH / 12 / 2)^2, na.rm = TRUE),
  ...
  left_join(
    nrshrv_plot_stats |>
      filter(PRE_HARVEST == 1) |>
      select(STAND_ID, BALIVE),
    by = join_by(StandID == STAND_ID)
  ) |>
  mutate(
    FVS_TO_FIA = StartingBA / BALIVE
  )
ba_vs_balive |>
  ggplot(aes(FVS_TO_FIA)) +
  geom_histogram(binwidth = 0.05)
```

Yes, lines up almost perfectly.

What about harvested BA?

```{r harvest basal area, eval = FALSE}
harvested_ba <- nrshrv_prsc_cutlist |>
  group_by(StandID) |>
  summarize(
    HarvestedBA = sum(TPA * pi * (DBH / 12 / 2)^2, na.rm = TRUE),
    .groups = "keep"
  ) |>
  ungroup() |>
  mutate(
    HarvestedBA = conv_multiunit(HarvestedBA, "ft2 / acre", "m2 / hectare")
  )
harvested_ba |>
  ggplot(aes(HarvestedBA)) +
  geom_histogram()
```

Let's look at % of BA harvested by starting BA

```{r harvest intensity vs starting basal area, eval = FALSE}
harvest_summary <- starting_ba |>
  left_join(harvested_ba, by = join_by(StandID)) |>
  mutate(
    HarvestedBA = coalesce(HarvestedBA, 0),
    HarvestFraction = HarvestedBA / StartingBA
  )
harvest_summary |> ggplot(aes(StartingBA, HarvestedBA)) +
  geom_point(size = 1, color = color_palette[7]) +
  geom_abline(slope = 1, intercept = c(0,0), linewidth = 0.5, linetype = "dashed") +
  theme_bw() +
  theme(aspect.ratio = 1) +
  coord_cartesian(xlim = c(0, 60), ylim = c(0, 60))
```

```{r harvest plotting breakpoints, eval = FALSE}
summary(harvest_summary)
```

Quartile breaks for StartingBA are roughly c(90, 115, 140)
Quartile breaks for HarvestFraction are roughly c(0.1, 0.2, 0.3)

```{r harvest summary categories, eval = FALSE}
harvest_summary <- harvest_summary |>
  mutate(
    StandBA = ordered(case_when(
      StartingBA < 22 ~ "< 22",
      StartingBA < 28 ~ "22 - 28",
      StartingBA < 35 ~ "28 - 35",
      .default = ">= 35"
    ), levels = c("< 22", "22 - 28", "28 - 35", ">= 35")),
    HarvestIntensity = ordered(case_when(
      HarvestFraction < 0.20 ~ "< 0.20",
      HarvestFraction < 0.35 ~ "0.20 - 0.40",
      HarvestFraction < 0.55 ~ "0.40 - 0.60",
      .default = ">= 0.60"
    ), levels = c("< 0.20", "0.20 - 0.40", "0.40 - 0.60", ">= 0.60"))
  )
```

Do we get a more or less even breakdown?


```{r starting ba distribution, eval = FALSE}
harvest_summary |>
  group_by(StandBA) |>
  summarize(N = n()) |>
  ungroup() |>
  ggplot(aes(StandBA, N)) +
  geom_col()
```

```{r harvest intensity distribution, eval = FALSE}
harvest_summary |>
  group_by(HarvestIntensity) |>
  summarize(N = n()) |>
  ungroup() |>
  ggplot(aes(HarvestIntensity, N)) +
  geom_col()
```

Seems like a reasonable compromise between dead-on quartiles and aesthetic breaks.
...
nrshrv_prsc_carbon |>
ggplot(aes(x = YearsSinceHarvest, y = Aboveground_Total_Live)) +
...
which is lovely, but hard to read.

```{r how to handle projected time, eval = FALSE}
time_distribution <- nrshrv_prsc_carbon |>
  group_by(StandID) |>
  arrange(Year) |>
  mutate(MeasNum = row_number()) |>
  ungroup()
  ggplot(time_distribution, aes(factor(MeasNum), YearsSinceHarvest)) +
  geom_boxplot() +
  ggplot(time_distribution, aes(MeasNum)) +
  geom_histogram(binwidth = 0.25)
  
```

Which makes it look like we should use year breakpoints of c(-10, 0, 10, 20),
calling out 0 as a special case, so 10<0, 0, 0<10, 10<20, 20+.

```{r time buckets, eval = FALSE}
nrshrv_prsc_carbon |>
  mutate(
    TimeStep = ordered(
      case_when(
        YearsSinceHarvest < 0 ~ "Pre-Harvest",
        YearsSinceHarvest == 0 ~ "Harvest",
        YearsSinceHarvest < 5 ~ "1 - 5",
        YearsSinceHarvest < 10 ~ "5 - 10",
        YearsSinceHarvest < 15 ~ "10 - 15",
        YearsSinceHarvest < 20 ~ "15 - 20",
        .default = "20+"
      ),
      levels = c("Pre-Harvest", "Harvest", "1 - 5", "5 - 10", "10 - 15", "15 - 20", "20+")
    )
  ) |>
  left_join(harvest_summary, by = join_by(StandID)) |>
  ggplot(aes(TimeStep, Aboveground_Total_Live, fill = HarvestIntensity)) +
  facet_wrap("HarvestIntensity") +
  geom_boxplot(outlier.size = 0.1) +
  theme_bw() +
  scale_fill_discrete(name = "Harvest Intensity") +
...
Do BAI as well - can we get pre-harvest?

Less than 30% of plots have a PREV_PLT_CN, so for the most part we can't go back
prior to the first inventory. We can, however, estimate the BAI up to harvest
by:

- for trees not harvested, take post-harvest DBH minus pre-harvest DBH / delta MEASYEAR
- for trees harvested, take harvest DBH minus pre-harvest DBH / (HRVYR - MEASYEAR)

```{r first increment bai, eval = FALSE}
harvested_plots <- nrshrv_plot_stats |>
  filter(HARVEST == 1) |>
  distinct(CN, STATECD, COUNTYCD, PLOT, INVYR)

# What can we learn about trees that survived from one inventory to the next
trees_in_harvest_inventory <- fia_trees_filtered(fiadb, harvested_plots, \(.data, con) {
  .data |>
    select(
      CN, STATECD, COUNTYCD, PLOT, INVYR, MORTYR,
      STATUSCD, TPA_UNADJ, DIA, PREVDIA
    )
})

# For ingrowth, we can fill in PREVDIA from the GRM table
# For harvest, we can fill in DIA from the GRM table; we will also need
# to adjust MEASYEAR to HRVYR
grm_dia <- fia_grm_estn(fiadb, trees_in_harvest_inventory) |>
  # Each TRE_CN will have multiple rows with different estimation types;
  # they all have the same diameter information, so pick one record
  filter(LAND_BASIS == "FORESTLAND" & ESTN_TYPE == "AL") |>
  group_by(TRE_CN) |>
  filter(row_number() == 1) |>
  ungroup() |>
  select(TRE_CN, DIA_BEGIN, DIA_MIDPT, DIA_END, TPAGROW_UNADJ) |>
  rename(CN = TRE_CN)
```

Let's gather:
1. The BAI of the stand at the start
   a. FIA provides BALIVE for the entire stand; this may or may not
      line up with what we want
   b. In the first post-harvest inventory, we can find the start / end BA of all
      non-harvested trees, and the start / harvested BA of all harvested trees,
      then divide by the measurement interval or measurement-to-harvest interval
      to get stand-level BAI
2. The BAI of the stand for each subsequent post-harvest inventory
   a. From the second post-harvest inventory onward, we can sum the BA of all
      trees from start to finish to get BAI
3. We can then plot Pre-harvest BAI, and post-harvest BAI 1 & 2

```{r pre_harvest_bai, eval=FALSE}
pre_harvest_bai_trees <- trees_in_harvest_inventory |>
  left_join(
    nrshrv_plot_stats |>
      filter(HARVEST == 1) |>
      select(STATECD, COUNTYCD, PLOT, MEASYEAR, HRVYR),
    by = join_by(STATECD, COUNTYCD, PLOT)
  ) |>
  left_join(
    nrshrv_plot_stats |>
      ...
      rename(PREV_MEASYEAR = MEASYEAR),
    by = join_by(STATECD, COUNTYCD, PLOT)
  ) |>
  left_join(grm_dia, by = join_by(CN)) |>
  mutate(
    PREVDIA = coalesce(DIA_BEGIN, PREVDIA, DIA),
    DIA = coalesce(DIA_MIDPT, DIA, PREVDIA), # Harvested trees get midpoint diameter
    MEASYEAR = case_when(
      STATUSCD == 3 ~ HRVYR, # Ending diameter is as of harvest
      STATUSCD == 2 ~ HRVYR, # Ending diameter is as of death, presumed to be close to harvest
      .default = MEASYEAR
    ),
    # It can happen that previous measurement and harvest happened in the same
    # year; in that case, add a year to the second measurement so that they
    # don't happen in the same year.
    MEASYEAR = case_when(
      MEASYEAR == PREV_MEASYEAR ~ MEASYEAR + 1,
      .default = MEASYEAR
    ),
    TPA_UNADJ = coalesce(
      TPAGROW_UNADJ, # Harvested trees
      TPA_UNADJ,
      case_when(
        DIA >= 5 ~ 6.018046,
        .default = 74.965282
    ...
    BA_START = TPA_UNADJ * pi * (PREVDIA / 12 / 2)^2,
    BA_END = TPA_UNADJ * pi * (DIA / 12 / 2)^2,
    BA_DELTA = BA_END - BA_START,
    BAI = BA_DELTA / (MEASYEAR - PREV_MEASYEAR)
  ) |>
  filter(!is.na(DIA) & !is.na(PREVDIA)) # Trees that have been dead for two inventories

pre_harvest_bai <- pre_harvest_bai_trees |>
  group_by(STATECD, COUNTYCD, PLOT) |>
  summarize(
    MEASYEAR = max(MEASYEAR),
    PREV_MEASYEAR = min(PREV_MEASYEAR),
    HRVYR = min(HRVYR),
    BA_START = sum(BA_START),
    BA_END = sum(BA_END),
    BA_DELTA = sum(BA_DELTA),
    BAI = sum(BAI),
    .groups = "keep"
  ) |>
  ungroup() |>
  ...
  left_join(
    nrshrv_plot_stats |>
      filter(HARVEST == 1) |>
      select(STATECD, COUNTYCD, PLOT, BALIVE) |>
      rename(BALIVE_END_FIA = BALIVE),
    by = join_by(STATECD, COUNTYCD, PLOT)
  )
```

NOTE: We'd love to compute this relative to FIA's BALIVE in the pre-harvest
inventory, but we need to compute each individual tree's BAI because they
live for different amounts of time (harvest, mortality), and FIA only gives
stand-level BALIVE, not tree-level.

This gets us within 25% of BALIVE in the pre-harvest inventory.
```{r pre_harvest_bai_error, eval = FALSE}
ba_err <- pre_harvest_bai |>
  mutate(
    BA_START_ERROR = BALIVE_FIA - BA_START,
    BA_START_ERRPCT = BA_START_ERROR / BA_START,
    BA_START_PROP = BA_START / BALIVE_FIA
  )
ggplot(ba_err, aes(BA_START_ERROR)) +
  geom_histogram(binwidth = 10) +
  ggplot(ba_err, aes(BA_START_PROP)) +
  geom_histogram(binwidth = 0.05)
```

What BAI do we compute for the pre-harvest inventory:
```{r plot pre harvest bai, eval = FALSE}
pre_harvest_bai |>
  ggplot(aes(BAI)) +
  geom_histogram(binwidth = 0.5)
```

```{r bai fvs, eval = FALSE}
fvs_bai <- nrshrv_prsc_treelist |>
  mutate(
    PREV_DBH = DBH - DG,
    BA = TPA * pi * (DBH / 12 / 2)^2,
    PREV_BA = TPA * pi * (PREV_DBH / 12 / 2)^2,
    BAI = (BA - PREV_BA) / PrdLen
  ) |>
  group_by(StandID, Year) |>
  summarize(BAI = sum(BAI), .groups = "keep") |>
  ungroup() |>
  left_join(
    nrshrv_plot_stats |> filter(HARVEST == 1) |> select(STAND_ID, STATECD, COUNTYCD, PLOT, HRVYR),
    by = join_by(StandID == STAND_ID)
  ) |>
  filter(Year > HRVYR) |>
  select(StandID, Year, HRVYR, BAI) |>
  mutate(Source = "FVS") |>
  union_all(
    pre_harvest_bai |>
      left_join(
        nrshrv_plot_stats |> select(STATECD, COUNTYCD, PLOT, STAND_ID),
        by = join_by(STATECD, COUNTYCD, PLOT)
      ) |>
      select(STAND_ID, PREV_MEASYEAR, HRVYR, BAI) |>
      rename(StandID = STAND_ID, Year = PREV_MEASYEAR) |>
      mutate(Source = "Estimated")
  ) |>
  mutate(
    YearsSinceHarvest = Year - HRVYR,
    TimeStep = ordered(
      case_when(
        YearsSinceHarvest < 0 ~ "Pre-Harvest",
        YearsSinceHarvest == 0 ~ "Harvest",
...
```{r plot fvs bai, eval = FALSE}
fvs_bai |>
  filter(YearsSinceHarvest < 20 & YearsSinceHarvest != 0) |>
  mutate(BAI = conv_multiunit(BAI, "ft2 / acre", "m2 / hectare")) |>
  ggplot(aes(TimeStep, BAI, fill = Source)) +
  facet_wrap("HarvestIntensity", labeller = label_both) +
  geom_boxplot(outlier.size = 0.1) +
  theme_bw() +
  scale_fill_discrete(name = "BAI Source") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle("FVS Projected BAI in Harvested Stands") +
  xlab("Years since Harvest") +
  ylab(bquote("BAI" ~ (m^2 %.% hectare^-1 %.% year^-1)))
...
Also lovely, also impossible to interpret, so let's see how we should deal with time.

```{r how to handle measured time, eval = FALSE}
time_for_measurement <- nrshrv_srvy_carbon |>
  group_by(StandID) |>
  arrange(Year) |>
  mutate(MeasNum = row_number()) |>
  ungroup()
ggplot(time_for_measurement, aes(factor(MeasNum), YearsSinceHarvest)) +
  geom_boxplot() +
  ggplot(time_for_measurement, aes(MeasNum)) +
  geom_histogram(binwidth = 0.25)
  
```

So, much more clearly in 5-year increments (no surprise).

```{r mesured time buckets, eval = FALSE}
nrshrv_srvy_carbon |>
  mutate(
    TimeStep = ordered(
      case_when(
...
Remove mountain ash plots?

Let's get subsequent increments of BA and BAI.

```{r post harvest bai fia, eval = FALSE}
prev_inv_mixin <- nrshrv_plot_stats |>
  select(STATECD, COUNTYCD, PLOT, INVNUM, MEASYEAR, BALIVE) |>
  mutate(INVNUM = INVNUM + 1) |>
  rename(
...
harvest_year_mixin <- nrshrv_plot_stats |>
  filter(HARVEST == 1) |>
  select(STATECD, COUNTYCD, PLOT, HRVYR)

post_harvest_bai_fia <- nrshrv_plot_stats |>
  filter(HARVEST == 0 & PRE_HARVEST == 0) |>
  left_join(
    prev_inv_mixin,
    by = join_by(STATECD, COUNTYCD, PLOT, INVNUM)
  ) |>
  ...
  select(STATECD, COUNTYCD, PLOT, MEASYEAR, HRVYR, BAI, Source) |>
  left_join(
    nrshrv_plot_stats |>
      filter(PRE_HARVEST == 1) |>
      select(STATECD, COUNTYCD, PLOT, STAND_ID) |>
      rename(StandID = STAND_ID),
    by = join_by(STATECD, COUNTYCD, PLOT)
  ) |>
  select(StandID, BAI, Source, MEASYEAR, HRVYR) |>
  union_all(
    # Fill in the two missing rows with values that won't show in the chart
    data.frame(
      StandID = c("002300300203", "000900300230"),
      BAI = c(9999, 9999),
      Source = c("FIA", "FIA"),
      MEASYEAR = c(1, 1),
      HRVYR = c(0, 0)
  ...
  select(StandID, TimeStep, YearsSinceHarvest, HarvestIntensity, StandBA, BAI, Source) |>
  union_all(
    fvs_bai |>
      select(StandID, TimeStep, YearsSinceHarvest, HarvestIntensity, StandBA, BAI, Source)
  )
...
```{r plot post-harvest bai fia, eval = FALSE}
post_harvest_bai_fia |>
  filter(YearsSinceHarvest < 20 & YearsSinceHarvest != 0) |>
  mutate(BAI = conv_multiunit(BAI, "ft2 / acre", "m2 / hectare")) |>
  ggplot(aes(TimeStep, BAI, fill = Source)) +
  geom_boxplot(outlier.size = 0.1) +
  ...
  ggtitle("BAI in Harvested Stands") +
  xlab("Years since Harvest") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ylab(bquote("BAI" ~ (m^2 %.% hectare^-1 %.% year^-1))) +
...
```{r plot post-harvest bai fia by starting ba, eval = FALSE}
post_harvest_bai_fia |>
  filter(YearsSinceHarvest < 20 & YearsSinceHarvest != 0) |>
  mutate(BAI = conv_multiunit(BAI, "ft2 / acre", "m2 / hectare")) |>
  ggplot(aes(TimeStep, BAI, fill = Source)) +
  geom_boxplot(outlier.size = 0.1) +
  facet_wrap("StandBA", labeller = label_both) +
  theme_bw() +
  scale_fill_discrete(name = "Starting Stand BA") +
  ggtitle("Estimated BAI in Harvested Stands") +
  xlab("Years since Harvest") +
  ylab(bquote("BAI" ~ (ft^2 %.% acre^-1 %.% year^-1))) +
  coord_cartesian(ylim = c(-2, 6))
```
