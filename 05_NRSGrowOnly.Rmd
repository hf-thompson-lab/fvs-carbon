...
### NRS GrowOnly NONE Carbon

```{targets nrsgrowonly_none_carbon, tar_simple = TRUE}
...
### NRS GrowOnly NONE Summary

```{targets nrsgrowonly_none_summary, tar_simple = TRUE}
...
### NRS GrowOnly SRVY Carbon

```{targets nrsgrowonly_srvy_carbon, tar_simple = TRUE}
fvs_read_output(nrs_growonly_srvy, "FVS_Carbon") |>
  select(StandID, Year, Aboveground_Total_Live)
```

### NRS GrowOnly SRVY Summary

```{targets nrsgrowonly_srvy_summary, tar_simple = TRUE}
...
decadal_carbon_tmp <- nrsgrowonly_none_carbon |>
...
nrsgrowonly_projected_vs_measured has the projection and measurement for the quantities
of interest in the first and last year, and nothing in between.

```{targets nrsgrowonly_projected_vs_measured, tar_simple = TRUE}
...
ggplot(
    nrsgrowonly_projected_vs_measured |> filter(Projection_Years > 0),
...
nrsgrowonly_projected_vs_measured |>
  filter(Projection_Years > 0) |>
...
```{r summary nrsgrowonly_projected_vs_measured, eval = FALSE}
nrsgrowonly_projected_vs_measured |>
  filter(Projection_Years > 0) |>
...
all_years <- tibble(Year = min(nrsgrowonly_none_carbon$Year):max(nrsgrowonly_none_carbon$Year))
...
nrsgrowonly_none_carbon |>
  ...
  full_join(
    expand.grid(
      StandID = unique(nrsgrowonly_none_carbon$StandID),
      Year = all_years$Year
    ),
...
all_years <- tibble(Year = min(nrsgrowonly_srvy_carbon$Year):max(nrsgrowonly_srvy_carbon$Year))
num_years <- all_years |> nrow()

nrsgrowonly_srvy_carbon |>
  group_by(StandID) |>
  arrange(Year) |>
  mutate(
    C_Flux = -(Aboveground_Total_Live - lag(Aboveground_Total_Live)),
  ...
  full_join(
    expand.grid(
      StandID = unique(nrsgrowonly_projected_vs_measured$StandID),
      Year = all_years$Year
    ),
    by = join_by(StandID, Year)
  ) |>
...
ggplot(
    nrsgrowonly_projected_vs_measured |> filter(Projection_Years > 0) |> arrange(Projection_Years),
...
lr <- lm(
  ...
  data = nrsgrowonly_projected_vs_measured |> filter(Projection_Years > 0)
)
ggplot(
    nrsgrowonly_projected_vs_measured |> filter(Projection_Years > 0),
...
projected_vs_measured_with_n <- nrsgrowonly_projected_vs_measured |>
  filter(Projection_Years > 0) |>
  group_by(FOREST_TYPE_GROUP) |>
...
ggplot(
  ...
  # scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  geom_point(
    data = nrsgrowonly_projected_vs_measured |>
      filter(Projection_Years > 0) |> 
      group_by(FOREST_TYPE_GROUP) |> 
      summarize(
        Measured_Carbon_Flux = mean(Measured_Carbon_Flux),
...
ggplot(
    nrsgrowonly_projected_vs_measured |> filter(Projection_Years > 0) |> filter(QMD_METRIC < 100),
...
ggplot(
    nrsgrowonly_projected_vs_measured |> filter(BA_Error < 1),
