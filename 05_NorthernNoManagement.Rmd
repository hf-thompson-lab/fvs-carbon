...
There are ~30,000 trees that ingrow over the 20 years, so about one tree per
plot per year.
Most of these trees are over the 3"DBH diameter limit on ingrowth set by FVS.

##### Ingrown Tree History

Trees >1" DBH are in the TREE table in FVS, so in some cases we should be able
to follow a tree back to the point when it was <=3"DBH.
...
fia_tree_cols <- function(.data) {
  .data |>
    select(
      ...
      INVYR, MEASYEAR, STATECD, UNITCD, PLOT, SUBP, TREE, CONDID,
...
# SQLite3 takes forever to do the semi_join at scale (despite joining on an index),
# so we run in chunks of 1000.
...
dbDisconnect(fia)
remove(fia, plt_cns, measyear_mixin, fia_tree, prev_tre_cns, progress_total)
```
...
For the remaining ~30,000 trees, we will need to back-project. TREE_GRM_COMPONENT
...
```{r growth_rate}
ingrowth_rate <- starting_trees |>
  filter(FRST_DIA < DIA) |>
  # Add on MEASYEAR for BEGIN
  left_join(prev_tree |> select(CN, MEASYEAR), by = join_by(CN)) |>
  rename(YEAR_END = MEASYEAR) |>
  # Add on MEASYEAR for END
  left_join(prev_tree |> select(CN, MEASYEAR), by = join_by(FRST_TRE_CN == CN)) |>
  rename(YEAR_BEGIN = MEASYEAR) |>
  # Add on growth rate
  left_join(ingrowth |> select(TRE_CN, ANN_DIA_GROWTH), by = join_by(CN == TRE_CN)) |>
  mutate(DIA_PROJ = DIA - (YEAR_END - YEAR_BEGIN) * ANN_DIA_GROWTH) |>
  mutate(DIA_ERR = (DIA_PROJ - FRST_DIA) / FRST_DIA) |>
  arrange(DIA_ERR)
ingrowth_rate |> filter(abs(DIA_ERR) < 0.1) |> nrow() / ingrowth_rate |> nrow()
```

This shows that using the FIA growth rate, only 30% of trees back-project
...
Let's see if we can compute our own growth rate from a subsequent tree record.
