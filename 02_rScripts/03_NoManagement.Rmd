...
```{r}
ggplot(
    data = projected_carbon |> union_all(FIA_Measured),
...
projected_observed <- projected_carbon |>
  ...
  select(PLOT_ID, Years, Projected, Measured)
...
ggplot(
  ...
  geom_point(
    size = 2,
    mapping = aes(x = Measured, y = Projected, color = Years)
  ) +
  ggtitle("FVS NoManagement Scenario\nMeasured vs. Projected Carbon (Mg C/ha)") +
