...
```{targets nrsgro_plot, tar_simple = TRUE}
...
```{r count nrsgro_plot, eval = FALSE}
nrsgro_plot |>
...
```{targets nrsgro_plot_stats, tar_simple = TRUE}
...
plot_location_tmp <- nrsgro_plot |>
  left_join(
    nrsgro_plot_stats |> select(STATECD, COUNTYCD, PLOT, INVYR, FOREST_TYPE_GROUP),
...
ggplot(fvsne_states_map, aes(long, lat, group = group)) +
  ...
  ggtitle("Unharvested, Undisturbed FIA Plots in the Northeast Region") +
  xlab("Longitude") +
  ylab("Latitude")
...
```{targets nrsgro_grm_all, tar_simple = TRUE}
fia_grm_ingrowth(fiadb, nrsgro_plot)
```

Fetch the corresponding trees

```{targets nrsgro_tree_grm_all, tar_simple = TRUE}
...
```{r nrsgro_tree_grm_dupes, eval = FALSE}
nrsgro_tree_grm_all |>
...
```{targets nrsgro_grm, tar_simple = TRUE}
...
```{targets nrsgro_tree_grm, tar_simple = TRUE}
multi_ingrowth <- nrsgro_tree_grm_all |>
  group_by(STATECD, COUNTYCD, PLOT, SUBP, TREE) |>
  arrange(INVYR) |>
  filter(
...
```{r preview nrsgro_grm, eval = FALSE}
print(paste0("Ingrown trees: ", nrsgro_grm |> nrow()))
nrsgro_grm_with_measyear <- nrsgro_grm |>
  left_join(nrsgro_plot |> select(CN, MEASYEAR), by = join_by(PLT_CN == CN))
print(paste0(
  "  ",
  nrow(nrsgro_grm) /
    nrow(nrsgro_plot) /
    (max(nrsgro_grm_with_measyear$MEASYEAR) - min(nrsgro_grm_with_measyear$MEASYEAR)),
  " ingrown trees / plot / year"
))
print(paste0("Ingrown trees over 3\" DBH: ", nrsgro_grm |> filter(DIA_END > 3) |> nrow()))
```
...
```{r nrsgro_grm_histogram, eval = FALSE}
nrsgro_grm_with_measyear |>
  mutate(
...
```{r nrsgro_grm_boxplot, eval = FALSE}
nrsgro_grm_with_measyear |>
  select(DIA_END) |>
  ggplot(aes(DIA_END)) +
  geom_histogram(binwidth = 1) +
...
```{targets nrsgro_tree, tar_simple = TRUE}
nrsgro_tree <- fia_trees(fiadb, nrsgro_plot) |>
    filter(CONDID == 1) # only live trees
```

```{targets nrsgro_tree_history, tar_simple = TRUE}
...
nrsgro_tree_history |>
  semi_join(
    nrsgro_tree_history |> filter(ESTAB == TRUE),
...
tree_growth_fia <- nrsgro_tree_history |>
  left_join(
    nrsgro_grm |> select(TRE_CN, ANN_DIA_GROWTH),
...
```{targets nrsgro_tree_growth, tar_simple = TRUE}
...
tree_growth_with_outliers <- nrsgro_tree_growth |>
  filter(!is.na(ANN_DIA_DELTA)) |>
  group_by(SPCD) |>
  arrange(ANN_DIA_DELTA) |>
  mutate(
...
nrsgro_tree_growth |>
  left_join(nrsgro_tree_history |> select(CN, FIRST_TRE_CN, ESTAB), by = join_by(CN)) |>
  group_by(FIRST_TRE_CN) |>
  arrange(MEASYEAR) |>
  mutate(
...
manual_back_projection <- nrsgro_tree_growth |>
  # Start with actually ingrown trees
  semi_join(nrsgro_tree_history |> filter(ESTAB), by = join_by(CN)) |>
...
projected_annual_growth <- nrsgro_tree_growth |>
  left_join(nrsgro_tree_history |> select(CN, FIRST_TRE_CN, ESTAB), by = join_by(CN)) |>
  group_by(FIRST_TRE_CN) |>
  arrange(MEASYEAR) |>
  mutate(
...
tree_ingrown_three <- nrsgro_tree_growth |>
  # Start with actually ingrown trees
  semi_join(nrsgro_grm, by = join_by(CN == TRE_CN)) |>
...
tree_ingrown_three_validation <- tree_ingrown_three |>
  select(
    CN, MEASYEAR, SPCD, # STATECD, COUNTYCD, PLOT, SUBP, TREE, MEASYEAR, SPCD,
...
```{targets nrsgro_tree_spcd, tar_simple = TRUE}
...
plot_count_mixin <- nrsgro_tree_spcd |>
  filter(is.na(FVS_SPCD)) |>
...
fvsne_unhandled_trees <- nrsgro_tree_spcd |>
  # Filter to the most recent record for each tree
  group_by(STATECD, COUNTYCD, PLOT, SUBP, TREE) |>
  arrange(desc(MEASYEAR)) |>
  filter(row_number() == 1) |>
  ungroup() |>
  mutate(BA = conv_multiunit(6 * pi * (DIA/12/2)^2, "ft2 / acre", "m2 / hectare")) |>
  group_by(STATECD, COUNTYCD, PLOT) |>
...
```{targets nrsgro_estab_rate, tar_simple = TRUE}
...
```{r plot-nrsgro_estab_rate, eval = FALSE}
stand_stats_mixin <- nrsgro_plot_stats |>
  group_by(STATECD, COUNTYCD, PLOT) |>
  arrange(MEASYEAR) |>
  filter(row_number() == 1) |>
  ungroup() |>
...
By species group:
```{r plot nrsgro_estab_rate by sp grp, eval = FALSE}
nrsgro_estab_rate |>
  left_join(stand_stats_mixin, by = join_by(STATECD, COUNTYCD, PLOT)) |>
  ggplot(aes(x = QMD, y = RATE_PER_HECTARE)) +
  geom_bin2d(bins = 50) +
...
```{targets nrsgro_estab_height, tar_simple = TRUE}
min_tree_count <- 3
max_tree_count <- 9
...
```{r plot nrsgro_estab_height origin, eval = FALSE}
nrsgro_estab_height |>
  mutate(
    HT_ORIGIN = case_when(
      !is.na(HT_PLOT) ~ "Plot",
...
nrsgro_estab_height_whence <- trees_for_ht_estimation |>
...
bins <- (max(nrsgro_estab_height_whence$DIA) - min(nrsgro_estab_height_whence$DIA)) * 10
nrsgro_estab_height_whence |>
...
stopifnot(
  nrsgro_estab_rate |>
    distinct(STATECD, COUNTYCD, PLOT, SPCD) |>
    left_join(nrsgro_estab_height, by = join_by(STATECD, COUNTYCD, PLOT, SPCD)) |>
...
ingrowth_by_stem <- nrsgro_estab_rate |>
  left_join(nrsgro_estab_height, by = join_by(STATECD, COUNTYCD, PLOT, SPCD))
...
by (DIA/2)^2 / 3^2 and set the height to (3/DIA)*HT - this would give us the same
...
```{targets fvs_num_partitions, tar_simple = TRUE}
...
```{targets fvs_partition, tar_simple = TRUE}
1:fvs_num_partitions
```
...
```{targets fvs_randseed, tar_simple = TRUE}
...
********   FVS16 ERROR:  KEYWORD ENTERED IS USED IN WRONG CONTEXT AND WAS IGNORED.  RECORDS READ=4884
           DSNOUT DATA BASE CAN NOT BE REDEFINED. DSN FOR OUTPUT REMAINS: FVS_nrsgro_GROW_27.db
```
...
```
STDIDENT
           STAND ID= 338300300218_Y1_R1                nrsgro                                                             
...
```{targets nrsgro_none_carbon, tar_simple = TRUE}
...
```{targets nrsgro_none_summary, tar_simple = TRUE}
...
```{targets nrsgro_srvy_carbon, tar_simple = TRUE}
...
```{targets nrsgro_srvy_summary, tar_simple = TRUE}
...
matched_carbon_tmp <- nrsgro_plot |>
  group_by(STATECD, COUNTYCD, PLOT) |>
  summarize(
    FIRST_YEAR = min(MEASYEAR, na.rm = TRUE),
    LAST_YEAR = max(MEASYEAR, na.rm = TRUE),
...
decadal_carbon_tmp <- nrsgro_none_carbon |>
...
nrsgro_none_proj_vs_meas has the projection and measurement for the quantities
of interest in the first and last year, and nothing in between.

```{targets nrsgro_none_proj_vs_meas, tar_simple = TRUE}
...
ggplot(
  nrsgro_none_proj_vs_meas |> filter(Projection_Years > 0),
...
```{r summary nrsgro_none_proj_vs_meas, eval = FALSE}
nrsgro_none_proj_vs_meas |>
  filter(Projection_Years > 0) |>
...
all_years <- tibble(Year = min(nrsgro_none_carbon$Year):max(nrsgro_none_carbon$Year))
...
nrsgro_none_carbon |>
  ...
  full_join(
    expand.grid(
      StandID = unique(nrsgro_none_carbon$StandID),
      Year = all_years$Year
    ),
...
all_years <- tibble(Year = min(nrsgro_srvy_carbon$Year):max(nrsgro_srvy_carbon$Year))
num_years <- all_years |> nrow()
...
nrsgro_srvy_carbon |>
  ...
  full_join(
    expand.grid(
      StandID = unique(nrsgro_none_proj_vs_meas$StandID),
      Year = all_years$Year
    ),
    by = join_by(StandID, Year)
  ) |>
...
ggplot(
  nrsgro_none_proj_vs_meas |> filter(Projection_Years > 0) |> arrange(Projection_Years),
...
#    limits = c(0, 125)
  ...
  labs(caption = "Without Calibration") +
...
Show us that stats:
```{r projected vs measured linear regression, eval = FALSE}
summary(lm(
  Projected_Carbon_Flux ~ Measured_Carbon_Flux,
  data = nrsgro_none_proj_vs_meas |> filter(Projection_Years > 0)
))
```
...
projected_vs_measured_with_n <- nrsgro_none_proj_vs_meas |>
  filter(Projection_Years > 0) |>
  group_by(FOREST_TYPE_GROUP) |>
...
```{r carbon flux residual vs ba, eval = FALSE}
nrsgro_none_proj_vs_meas |>
  left_join(
    nrsgro_plot_stats |>
      group_by(STATECD, COUNTYCD, PLOT) |>
  ...
  ggtitle("Carbon Flux Residual vs. Stand Starting Basal Area (No Calibration)") +
#  labs(caption = "Without Calibration") +
...
ggplot(
  nrsgro_none_proj_vs_meas |> filter(Projection_Years > 0) |> filter(QMD_METRIC < 100),
...
ggplot(
  nrsgro_none_proj_vs_meas |> filter(BA_Error < 1),
...
ggplot(
  nrsgro_none_proj_vs_meas |>
    ...
    mutate(FOREST_TYPE_GROUP = paste0(FOREST_TYPE_GROUP, ", n=", n())) |>
    ungroup(),
  aes(x = QMD_Range, y = BAI_Residual)
) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_boxplot(outlier.size = 0.1, varwidth = TRUE) +
  scale_fill_discrete(name = "Forest Type Group") +
...
ggplot(
  nrsgro_none_proj_vs_meas |>
    ...
    mutate(FOREST_TYPE_GROUP = paste0(FOREST_TYPE_GROUP, ", n=", n())) |>
    ungroup(),
  aes(x = Tpa_Range, y = BAI_Residual)
) +
  geom_hline(yintercept = 0, linetype = "dashed") +
...
missing_classes <- plot_eastern_hemlock |>
  distinct(CCL) |>
...
plot_eastern_hemlock <- eastern_hemlock |>
  ...
  # filter(CANOPY_HT > 30 & CANOPY_HT < 100) |>
  mutate(HT_GROUP = factor(as.integer(CANOPY_HT / 10) * 10))
missing_classes <- eastern_hemlock |>
  distinct(CCL) |>
  cross_join(plot_eastern_hemlock |> distinct(HT_GROUP)) |>
  anti_join(plot_eastern_hemlock |> distinct(HT_GROUP, CCL), by = join_by(HT_GROUP, CCL)) |>
