...
```{targets nk_ffe_none_carbon}
tar_target(nk_ffe_none_carbon, fvs_read_output(nk_ffe_none[3], "FVS_Carbon"))
```

We're not using the summary yet, but define the target for it.

```{targets nk_ffe_none_summary}
tar_target(nk_ffe_none_summary, fvs_read_output(nk_ffe_none[3], "FVS_Summary2_East"))
```
