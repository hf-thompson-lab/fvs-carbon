...
```{r plot disturbance history, eval = FALSE}
...
tblDWSPCFIPlotVisitDisturbances |>
  ...
  summarize(PlotVisitDisturbanceCode = max(PlotVisitDisturbanceCode, na.rm = TRUE)) |>
  ungroup() |>
  mutate(
  ...
  ungroup() |>
  group_by(MasterPlotID) |>
  summarize(MaxUndisturbInterval = max(undisturbed_interval)) |>
  ungroup() |>
...
qryDWSPCFIPlotVisitTreeDetail |>
  ...
  summarize(BA = sum(pi * (VisitTreeDIAM/24)^2, na.rm = TRUE)) |>
  ungroup() |>
  mutate(
...
qryDWSPCFIPlotVisitTreeDetail |>
  ...
  filter(!CFIHarvested & !CFIDisturbed) |>
  group_by(MasterPlotID, VisitYear) |>
  summarize(TPA = n() * conv_unit(1, "acre", "ft2") / (pi * 52.7^2)) |>
  ungroup() |>
  mutate(
    MasterPlotID = factor(MasterPlotID)
  ) |>
...
qryDWSPCFIPlotVisitTreeDetail |>
  ...
  left_join(cfigro_plot_harvested, by = join_by(MasterPlotID)) |>
  left_join(cfigro_plot_disturbed, by = join_by(MasterPlotID)) |>
  #filter(!CFIHarvested & !CFIDisturbed & VisitTreeDIAM > 0) |>
  #group_by(VisitCycle) |>
  mutate(BreakDiameter = case_when(
    VisitTreeDIAM < 5 ~ "< 5\"",
    VisitTreeDIAM < 5.6 ~ "< 5.6\"",
    VisitTreeDIAM < 6.0 ~ "< 6\""
  )) |>
  filter(!is.na(BreakDiameter)) |>
  group_by(BreakDiameter) |>
  summarize(
    MinDiam = min(VisitTreeDIAM, na.rm = TRUE),
    SmallTrees = sum(if_else(VisitTreeDIAM < 5.6, 1, 0), na.rm = TRUE),
    AllTrees = n()
    )
```

Minimum diameter is 5.3" - this is important for BRK_DBH - trees smaller
...
From 1970 - 2020, only 2 trees ingrow on undisturbed, non-harvested plots with DIAM < 5.6"
...
Not really; compare to NRSGrowOnly:
...
What do we see if we just sum the BA in these plots?
```{r plot ba, eval = FALSE}
cfiabp_trees |>
  left_join(cfigro_plot_harvested, by = join_by(MasterPlotID)) |>
  left_join(cfigro_plot_disturbed, by = join_by(MasterPlotID)) |>
  filter(!CFIHarvested & !CFIDisturbed) |>
  ...
  group_by(MasterPlotID, VisitYear) |>
  summarize(BA = sum(pi * (dbhcm / 2)^2)) |>
  ungroup() |>
  mutate(MasterPlotID = factor(MasterPlotID)) |>
  ggplot(aes(VisitYear, BA, color = MasterPlotID)) +
  geom_line() +
  geom_point()
