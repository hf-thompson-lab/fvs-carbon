...
knitr::kable(fh_table_13)
```

For our purposes, the most interesting thing here is the relative
proportion, where 25% of reproduction is red maple, 21% is Oak NOS,
10% is white pine, and the rest is a scattering of other things.

If we assume 1 stem / acre / year of establishment, this means
that every 10 years, per acre, we should get:
- 3 red maple
- 2 oak
- 1 white pine
- 1 birch, probably black
- 1 sugar maple
- 1 white ash
- 1 hemlock
...
```
  ...
  mutate(
    VisitTreeTotalHeight = if_else(
      VisitTreeTotalHeight < 10, NA, VisitTreeTotalHeight
  )) |>
  group_by(MasterPlotID, SpeciesCode) |>
  # Some plots may have been recently harvested; remove the top 1%
  # by density to eliminate those
  mutate(
    ESTAB_COUNT = n()
  ) |>
  group_by(SpeciesCode) |>
  arrange(desc(ESTAB_COUNT)) |>
  filter(row_number() > (n() * 0.025)) |>
  ungroup() |>
  filter(ESTAB_COUNT > 0) |>
  group_by(SpeciesCode) |>
  summarize(
    # Stems per acre per 10-year interval
    DENSITY = mean(ESTAB_COUNT) * 5 / 10, # 5 plots per acre / 10 years
