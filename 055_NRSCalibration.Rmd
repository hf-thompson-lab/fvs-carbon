...
nrsgro_tree_dead |>
  ...
  mutate(
    DECAY = ordered(DECAY, c("Minimal", "Branches", "Sapwood", "Heartwood", "Advanced"))
  ) |>
...
with TREECLCD_NERS==6 ("Tree that has recently died (within the last several years)")
that are not in FVS_TreeInit_Plot as dead trees; they could all be added.

Does TREECLCD_NERS==6 correlate with DECAYCD==1?
...
nrsgro_tree_dead |>
  ...
  ggplot(aes(TREECLCD_NERS, DECAYCD)) +
  geom_bin2d()
```

lol no; TREECLCD_NERS==6 "recently ded with twigs" seem to correspond primarily
with DECAYCD==4 "few or no limb stubs remain" and DECAYCD==5 "no evidence of branches remain".
...
of recently dead trees in later inventories, so I'll add all trees
