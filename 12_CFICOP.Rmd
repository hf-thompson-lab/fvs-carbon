...
There's a handful of plots that FVS won't grow. Let's see what's up with them.
...
Stand 10353 has live trees on it (all WP) in 1970, but 0 biomass in 2020. Why?


```{r plot 10353 ba over time, eval = FALSE}
cficop_none_summary |>
  filter(StandID == 10353) |>
  mutate(Series = "Projected") |>
  mutate(Year = as.character(Year)) |>
  ggplot(aes(Year, BA, color = Series)) +
  geom_point()
...
a'ight, it has 0 biomass in all years. Why?
...
title <- "CFICarbonOffsetProgram"
mgmt_id <- "NONE"
fvs_warnings <- lapply(1:fvs_num_partitions, \(partition) {
  # Just read the first random seed
  rand_seed <- fvs_randseed[[1]]
  filename <- file.path(
    "data/fvs",
    paste0("FVS_", title, "_", mgmt_id),
    paste0("FVS_", title, "_", mgmt_id, "_P", partition, "_R", rand_seed, ".out")
  )
  read_lines(filename) |>
...
Find the warnings for plot 10353
```{r plot 10353 fvs warnings, eval = FALSE}
fvs_warnings_10353 <- fvs_warnings |>
...
From the log:
> TREESQL    TREESQL COMMAND FOR INPUT DATA BASE: FVS_CFICarbonOffsetProgram_NONE_P7_R43517_Input.db
>           SELECT * FROM FVS_TreeInit_Plot WHERE Stand_CN = '%Stand_CN%'
>           NUMBER ROWS PROCESSED:    0

So we're missing trees for some reason.

```{r trees for plot 10353, eval = FALSE}
cfigro_trees |> filter(STAND_ID == 10353) |> distinct(STAND_CN)
```

There are 4 STAND_CN for this plot. They all have live biomass, but there should
only be 1.

What do we have in plots?

```{r stand_cns for plot 10353, eval = FALSE}
cfigro_plot |> filter(STAND_ID == 10353) |> distinct(STAND_CN)
```
one STAND_CN, which does not match any of those of the trees. That would be
why we start with zero biomass.

```{r plot 10353 visits, eval = FALSE}
tblDWSPCFIPlotVisitsComplete |>
  filter(MasterPlotID == 10353) |>
  distinct(MasterPlotVisitID, VisitCycle)
```

Looks like MasterPlotVisitID is sprintf("%d%d%04d", trunc(MasterPlotID/1000), VisitCycle, MasterPlotID%1000)

```{r stand_cn for plot 10353 in 1970, eval = FALSE}
qryDWSPCFIPlotVisitTreeDetail |>
  filter(MasterPlotVisitID == 119700353)
```

So, in 1970, this plot had no live trees.

What about regen?
```{r stands without regen, eval = FALSE}
cficop_none_summary |>
  filter(Year == 2020) |>
  arrange(BA) |>
  head(4)
...
This particular plot is clearly a problem child when it comes to regen as well.
Not that plots 10361 and 10462 are doing great, but at least they get something.

```{r which partition stand 10353, eval = FALSE}
digest::digest2int(as.character(119700353)) %% fvs_num_partitions + 1
```

So, partition 7.

Looking at the keyword file, it gets no regeneration.

Why?

Who is its sibling plot?
```{r stand 10353 sibling, eval = FALSE}
cficop_plot_sibling |>
  filter(MasterPlotID == 10353)
```
...
```{r stands without siblings, eval = FALSE}
cfigro_plot |>
  anti_join(cficop_plot_sibling, by = join_by(STAND_ID == MasterPlotID)) |>
  select(STAND_ID)
```

6 times, which is probably 6 too many.
