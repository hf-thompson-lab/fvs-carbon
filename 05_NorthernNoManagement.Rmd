...
ingrowth_stem_rate |>
  ...
  xlab(bquote("QMD (cm)")) +
  ylab(bquote("Ingrowth Rate" ~(stems %*% hectare^-1 %*% year^-1))) +
...
```{r}
fia <- DBI::dbConnect(RSQLite::SQLite(), 'data/raw/SQLite_FIADB_ENTIRE.db')
plot_206 <- tbl(fia, 'COND') |>
  filter(STATECD == 42, COUNTYCD == 113, PLOT == 206) |>
  collect()
dbDisconnect(fia)
View(plot_206)
```
...
H = BH + B1*S^B2*(1-e^(B3*A))^(B4*S^B5)
where:
H = Height
BH = 0 when A is total age, and 4.5 when A is d.b.h age
S = site index
A = age
...
> TODO nik: How does FVS UI deal with stand replication? Should be able
> to re-jigger this to run everything in one pass.
> If necessary, output can be separated using the per-stand output
> specifications, but the UI should give a model for how to do replicas.
...
annual_vs_decadal |>
  ...
  mutate(Ratio = Annual_Carbon_Flux / Decadal_Carbon_Flux) |>
  ggplot(aes(x = Ratio)) +
  geom_histogram(bins = 50) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "red") +
#  facet_wrap(~`Forest Type Group`) +
  theme_bw() +
  ggtitle("Ratio of Projected Carbon using 1 vs. 10 year timestep") +
  ylab("Number of Plots") +
  xlab("Ratio (1-Year / 10-Year)")
...
```{r}
regen_vs_noregen |>
  mutate(Ratio = Carbon_Without_Regen / Carbon_With_Regen) |>
  ggplot(aes(x = Ratio)) +
  geom_histogram(bins = 50) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "red") +
  theme_bw() +
  ggtitle("Ratio of FVS Projected Carbon without vs. with regeneration") +
  ylab("Number of Plots") +
  xlab("Ratio of Projected Carbon Without to With Regeneration")
```

```{r noregen stats}
regen_vs_noregen |>
  mutate(
    Without_to_With = Carbon_Without_Regen / Carbon_With_Regen,
    With_to_Without = Carbon_With_Regen / Carbon_Without_Regen
  ) |>
  select("Carbon_With_Regen", "Carbon_Without_Regen", "Without_to_With", "With_to_Without") |>
  summary()
```
...
ggplot(
  ...
  ggtitle("Measured vs. Projected BA with Regeneration") +
