...
```{r species_mixin}
rds_file <- "data/intermediate/species_mixin.rds"
if (USE_CACHE & file.exists(rds_file)) {
  species_mixin <- read_rds(rds_file)
} else {
  ...
  write_rds(species_mixin, rds_file)
}
```
...
```{r plot_projected_annual_carbon_flux}
...
```{r plot_surveyed_annual_carbon_flux}
