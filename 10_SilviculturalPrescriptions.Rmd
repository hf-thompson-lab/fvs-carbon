...
load_result_table_helper <- function(title, mgmtid, partition, table) {
  ...
  fvs_carbon_tbl = tbl(fvs_out_db, table) |>
    collect() |>
    mutate(
      Title = title,
      MgmtId = mgmtid,
...
load_result_table <- function(title, mgmt_id, partition, table) {
  bind_rows(
    pbapply(
      expand.grid(MgmtId = mgmt_id, Partition = partition),
      1,
      \(row) {
        m <- row["MgmtId"]
        p <- as.integer(row["Partition"])
        load_result_table_helper(title, m, p, table)
      }
    )
  )
}
...
FVS_Carbon <- load_result_table(title, mgmt_ids, partitions, "FVS_Carbon")

print("Loading result Summary")
FVS_Summary2_East <- load_result_table(title, mgmt_ids, partitions, "FVS_Summary2_East")

print("Loading Treelist")
FVS_TreeList_East <- load_result_table(title, mgmt_ids, partitions, "FVS_TreeList_East")

rm(load_result_table, load_result_table_helper)
```
