...
if (file.exists(rds_file)) {
  ...
  species_mixin <- tbl(fia, 'REF_SPECIES') |>
    select(SPCD, GENUS, SPECIES, SCIENTIFIC_NAME, JENKINS_SPGRPCD)
  ...
  tree_growth <- tbl(fia, 'TREE') |>
    ...
    select(
      ...
      SPGRPCD, JENKINS_SPGRPCD, SPCD, GENUS, SPECIES, SCIENTIFIC_NAME,
  ...
  rm(fia, measyear_mixin, species_mixin, prev_tre_mixin, jenkins_spgrpcd_mixin)
...
y <- tree_growth |>
  ...
  mutate(
    ONE_LOWER_BOUND = n() * one_pct,
    TWO_LOWER_BOUND = n() * two_pct,
    ROW_NUMBER = row_number(),
    TWO_UPPER_BOUND = n() - n() * two_pct,
    ONE_UPPER_BOUND = n() - n() * one_pct,
    ONE_OUTLIER = (row_number() < (n() * one_pct)) | (row_number() > (n() - n() * one_pct)),
    TWO_OUTLIER = (row_number() < (n() * two_pct)) | (row_number() > (n() - n() * two_pct))
  ) |>
  ungroup()
x <- y |> mutate(OUTLIERS_REMOVED = "0%") |>
  union_all(y |> filter(!ONE_OUTLIER) |> mutate(OUTLIERS_REMOVED = "1%")) |>
  union_all(y |> filter(!TWO_OUTLIER) |> mutate(OUTLIERS_REMOVED = "2.5%")) |>
  group_by(OUTLIERS_REMOVED) |>
  mutate(OUTLIERS_REMOVED = paste0(OUTLIERS_REMOVED, ", n=", n())) |>
  ungroup()
```
...
ggplot(
    x,
    aes(ANN_DIA_GROWTH, color = OUTLIERS_REMOVED)
  ) +
  geom_boxplot() +
...
```{r tree_growth_rate}
filtered_growth <- tree_growth |>
  filter(!is.na(ANN_DIA_GROWTH)) |>
  group_by(SPCD) |>
  arrange(ANN_DIA_GROWTH) |>
  mutate(OUTLIER = (row_number() < (n() * 0.01)) | (row_number() > (n() - n() * 0.01))) |>
  ungroup() |>
  filter(!OUTLIER)

tree_growth_dom <- filtered_growth |>
  filter(CCLCD < 4) |>
  ...
  ungroup()
tree_growth_nondom <- filtered_growth |>
  filter(CCLCD > 3) |>
  group_by(STATECD, COUNTYCD, PLOT, MEASYEAR, SPCD) |>
  summarize(
  ...
  ungroup()
tree_growth_spcd <- filtered_growth |>
  group_by(STATECD, COUNTYCD, PLOT, MEASYEAR, SPCD) |>
  summarize(
  ...
  ungroup()
tree_growth_spgrpcd <- filtered_growth |>
  ...
  ungroup()
tree_growth_multiyear <- filtered_growth |>
  ...
  ungroup()
# prev_measyear_mixin does not use filtered_growth
# so that we don't project across multiple inventories
prev_measyear_mixin <- tree_growth |>
  ...
  ungroup()
tree_growth_rate <- filtered_growth |>
...
manual_back_projection <- tree_ingrown_prev |>
  ...
  # in the inventory in which they're marked as ingrowth
  semi_join(tree_ingrown |> select(TRE_CN), by = join_by(CN == TRE_CN)) |>
  filter(!is.na(PREV_TRE_CN)) |>
...
# Remove post-ingrowth inventories of ingrown trees
manual_back_projection <- manual_back_projection |>
  anti_join(
    manual_back_projection |> select(NEXT_TRE_CN) |> distinct(),
    by = join_by(CN == NEXT_TRE_CN)
  )
...
ggplot(
    projected_annual_growth |>
      select(CN, GROUP_NAME, ANN_DIA, ANN_DIA_PROJ) |>
      rename(Measured = ANN_DIA, Projected = ANN_DIA_PROJ) |>
      melt(
        id.vars = c("CN", "GROUP_NAME"),
        variable.name = "Method",
        value.name = "Diameter"),
    aes(Diameter, fct_rev(fct_infreq(GROUP_NAME)), color = Method)
  ) +
  geom_boxplot() +
