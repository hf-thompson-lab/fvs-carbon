...
starting_carbon <- cficop_none_bai |>
  # Remove random seed
  group_by(StandID) |>
  filter(row_number() == 1) |>
  ungroup() |>
  mutate(
#    Projected_Carbon = Starting_Carbon + (Projected_AGLCF) * (2020-1970),
    Adjusted_Carbon = Starting_Carbon + (Projected_AGLCF - Carbon_Flux_Error) * (2020 - 1970)
  ) |>
  summarize(
    starting_carbon_mean = mean(Starting_Carbon),
    starting_carbon_sd = sd(Starting_Carbon),
  ...
  mutate(
    starting_carbon = starting_carbon_mean,
    # since we multiply by hectares later, convert sd to ci via 1.96 / sqrt(n)
    starting_carbon_min = starting_carbon - 1.96 * starting_carbon_sd / sqrt(n),
    starting_carbon_max = starting_carbon + 1.96 * starting_carbon_sd / sqrt(n),
    projected_carbon = projected_carbon_mean,
    projected_carbon_min = projected_carbon - 1.96 * projected_carbon_sd / sqrt(n),
    projected_carbon_max = projected_carbon + 1.96 * projected_carbon_sd / sqrt(n),
    adjusted_carbon = adjusted_carbon_mean,
    adjusted_carbon_min = adjusted_carbon - 1.96 * adjusted_carbon_sd / sqrt(n),
    adjusted_carbon_max = adjusted_carbon + 1.96 * adjusted_carbon_sd / sqrt(n)
  ) |>
  select(
    starting_carbon, starting_carbon_min, starting_carbon_max,
  ...
  mutate(
    ...
    group = if_else(grepl("starting", name), "starting", if_else(grepl("projected", name), "projected", "adjusted")),
    kind  = if_else(grepl("_min", name), "min", if_else(grepl("_max", name), "max", "mean"))
  ) |>
  pivot_wider(id_cols = "group", names_from = "kind")
```

Plot it:
```{r}
starting_carbon |>
  mutate(group = ordered(group, levels = c("starting", "projected", "adjusted"))) |>
...
So, starting carbon is 50.89+/-2.49 Mg C / hectare, times hectares

To plot this, can use
- geom_bar
- geom_rect - set all positions by hand
- geom_linerange - set ymax and ymin, with a big size
- waterfall (in waterfall package)
...
projected_vs_corrected_carbon <- cficop_none_bai |>
  ...
  summarize(
    Starting = sum(StartingCarbon),
    FVS = sum(ProjectedCarbon),
    `FVS Adjusted` = sum(CorrectedCarbon),
    Error = (FVS - `FVS Adjusted`) / ((FVS + `FVS Adjusted`) / 2),
    GrowthError = (FVS - `FVS Adjusted`) / (FVS - Starting)
  )

knitr::kable(projected_vs_corrected_carbon, padding = 5)
```

```{r plot projected vs corrected carbon, eval = FALSE}
projected_vs_corrected_carbon |>
  select(
    Starting = StartingCarbon,
    FVS = ProjectedCarbon,
    `FVS Adjusted` = CorrectedCarbon
  ) |>
  pivot_longer(c("Starting", "FVS", "FVS Adjusted")) |>
  mutate(name = ordered(name, levels = c("Starting", "FVS", "FVS Adjusted"))) |>
  ggplot(aes(name, value)) +
  geom_boxplot() +
  theme_bw() +
  ggtitle("Modeled AGL Carbon") +
  xlab(label = NULL) +
