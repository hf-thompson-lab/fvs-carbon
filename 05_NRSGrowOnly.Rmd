...
nrsgro_projected_vs_measured has the projection and measurement for the quantities
of interest in the first and last year, and nothing in between.

```{targets nrsgro_projected_vs_measured, tar_simple = TRUE}
...
ggplot(
  nrsgro_projected_vs_measured |> filter(Projection_Years > 0),
...
```{r summary nrsgro_projected_vs_measured, eval = FALSE}
nrsgro_projected_vs_measured |>
  filter(Projection_Years > 0) |>
...
nrsgro_srvy_carbon |>
  ...
  full_join(
    expand.grid(
      StandID = unique(nrsgro_projected_vs_measured$StandID),
      Year = all_years$Year
    ),
    by = join_by(StandID, Year)
  ) |>
...
ggplot(
  nrsgro_projected_vs_measured |> filter(Projection_Years > 0) |> arrange(Projection_Years),
...
lr <- lm(
  ...
  data = nrsgro_projected_vs_measured |> filter(Projection_Years > 0)
)
ggplot(
  nrsgro_projected_vs_measured |> filter(Projection_Years > 0),
...
projected_vs_measured_with_n <- nrsgro_projected_vs_measured |>
  filter(Projection_Years > 0) |>
  group_by(FOREST_TYPE_GROUP) |>
...
ggplot(
  ...
  # scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  geom_point(
    data = nrsgro_projected_vs_measured |>
      filter(Projection_Years > 0) |>
      group_by(FOREST_TYPE_GROUP) |>
      summarize(
        Measured_Carbon_Flux = mean(Measured_Carbon_Flux),
...
ggplot(
  nrsgro_projected_vs_measured |> filter(Projection_Years > 0) |> filter(QMD_METRIC < 100),
...
ggplot(
  nrsgro_projected_vs_measured |> filter(BA_Error < 1),
