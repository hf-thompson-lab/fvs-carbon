...
AllPlots_Grow <- read_csv(
  ...
  mutate(Repro_Carbon = Aboveground_Live + Standing_Dead) |>
  select(StandID, Year, Total_Stand_Carbon,
         NK_Carbon, Repro_Carbon,
...
Avg_Grow <- AllPlots_Grow |>
  ...
  summarize(
    ...
    Repro_Carbon=mean(Repro_Carbon),
...
write_csv(Avg_Grow, "03_outputs/NoManagement_Carbon_ByYear.csv")
...
```{r}
Plots_RMSE <- tibble(
  Total_Carbon = sqrt(mean((Avg_Grow$Total_Carbon - Avg_Grow$NoManagement)^2)),
  NK_Carbon = sqrt(mean((Avg_Grow$NK_Carbon - Avg_Grow$NoManagement)^2)),
  Repro_Carbon = sqrt(mean((Avg_Grow$Repro_Carbon - Avg_Grow$NoManagement)^2))
)
ggplot(
...
ggplot(
    data = melt(AllSubplots_ByYear, id.vars = "Year"),
    mapping = aes(x = Year, y = value, color = variable)
) +
geom_line() +
coord_cartesian(xlim = c(2005, 2165), ylim = c(0, 300)) +
...
Subplots_RMSE <- tibble(
  Total_Carbon = sqrt(mean((AllSubplots_ByYear$Total_Carbon - AllSubplots_ByYear$NoManagement)^2)),
  NK_Carbon = sqrt(mean((AllSubplots_ByYear$NK_Carbon - AllSubplots_ByYear$NoManagement)^2)),
  Repro_Carbon = sqrt(mean((AllSubplots_ByYear$Repro_Carbon - AllSubplots_ByYear$NoManagement)^2))
)
ggplot(
...
360304303966 has 3 conditions

```{r}
AllConditions_ByYear <- read_csv(
...
ggplot(
    data = melt(AllConditions_ByYear, id.vars = "Year"),
    mapping = aes(x = Year, y = value, color = variable)
) +
geom_line() +
coord_cartesian(xlim = c(2005, 2165), ylim = c(0, 300)) +
...
```{r}
AllConditions_RMSE <- tibble(
  Total_Carbon = sqrt(mean((AllConditions_ByYear$Total_Carbon - AllConditions_ByYear$NoManagement)^2)),
  NK_Carbon = sqrt(mean((AllConditions_ByYear$NK_Carbon - AllConditions_ByYear$NoManagement)^2)),
  Repro_Carbon = sqrt(mean((AllConditions_ByYear$Repro_Carbon - AllConditions_ByYear$NoManagement)^2))
)
ggplot(
    data = melt(AllConditions_ByYear, id = NULL, variable.name = "Series", value.name = "RMSE"),
    mapping = aes(x = Series, y = RMSE)
  ) +
  geom_col()
```
