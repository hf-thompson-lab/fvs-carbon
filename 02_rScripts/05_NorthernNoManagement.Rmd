...
projected_vs_measured <- stand_stats |>
  ...
  mutate(BA_Error = BA_Residual / Measured_BA) |>
  mutate(BA_Error_Magnitude = abs(BA_Error)) |>
  mutate(Carbon_Residual = Projected_Carbon - Measured_Carbon) |>
  mutate(Carbon_Error = Carbon_Residual / Measured_Carbon) |>
  mutate(Carbon_Error_Magnitude = abs(Carbon_Error))
...
ggplot(
    projected_vs_measured |> filter(Carbon_Error_Magnitude <= 2),
...
```{r carbon_error_magnitude_vs_projection_years}
ggplot(
    projected_vs_measured |> filter(Carbon_Error_Magnitude <= 2),
    aes(x = Projection_Years, y = Carbon_Error_Magnitude)
  ) +
  geom_point(
    alpha = 0.25
  ) +
  ...
  facet_wrap(~`Forest Type Group`) +
  ggtitle("Grow-Only Stands\nCarbon Projection Error Magnitude (fraction) vs. Projection years")
...
ggplot(
  ...
  facet_wrap(~`Forest Type Group`) +
  ggtitle("Grow-Only Stands\nCarbon Projection Error (Mg/ha) vs. Measured BA")
...
ggplot(
    projected_vs_measured |>
      ...
      filter(Carbon_Error_Magnitude < 2),
...
ggplot(
  ...
  facet_wrap(~`Forest Type Group`) +
  ggtitle("Grow-Only Stands\nBA Projection Error (m²/ha) vs. Projection Years")
