...
stand_grow_only <- nk_matching_plot |>
  inner_join(rhs, by = join_by(STATECD, COUNTYCD, PLOT)) |>
  left_join(stand_num, by = join_by(STATECD, COUNTYCD, PLOT)) |>
...
```{targets nk_ffe_none, tar_simple = TRUE}
...
```{targets nk_ffe_srvy, tar_simple = TRUE}
fvsbin_dir <- "/fvs/fvsbin" # TODO: put these in a config file
fvs_variant <- "fvsne"      # TODO: put these in a config file
data_dir <- "data/fvs"
title <- "NKFFE"
...
fvs_read_output(nk_ffe_srvy, "FVS_Summary2_East")
```
...
```{targets nk_jenkins_none, tar_simple = TRUE}
fvsbin_dir <- "/fvs/fvsbin" # TODO: put these in a config file
fvs_variant <- "fvsne"      # TODO: put these in a config file
data_dir <- "data/fvs"
title <- "NKJenkins"
...
fvs_read_output(nk_jenkins_none, "FVS_Carbon")
```

We're not using the summary yet, but define the target for it.

```{targets nk_jenkins_none_summary, tar_simple = TRUE}
fvs_read_output(nk_jenkins_none, "FVS_Summary2_East")
```
...
```{targets nk_jenkins_srvy, tar_simple = TRUE}
fvsbin_dir <- "/fvs/fvsbin" # TODO: put these in a config file
fvs_variant <- "fvsne"      # TODO: put these in a config file
data_dir <- "data/fvs"
title <- "NKJenkins"
...
fvs_read_output(nk_jenkins_srvy, "FVS_Carbon")
```

We're not using the summary yet, but define the target for it.

```{targets nk_jenkins_srvy_summary, tar_simple = TRUE}
fvs_read_output(nk_jenkins_srvy, "FVS_Summary2_East")
```
...
compare_ffe_jenkins <- fvs_stand_measured_ffe |>
  ...
  select(StandID, Year, FFE, `Stand Age`, FOREST_TYPE) |>
  ...
  select(StandID, Year, FFE, Jenkins, `Stand Age`, FOREST_TYPE) |>
  group_by(StandID) |>
  filter(Year == max(Year))
...
ggplot(
  ...
  geom_point(
    size = 2,
    mapping = aes(x = FFE, y = Jenkins, color = Year, shape = FOREST_TYPE)
  ) +
...
ggplot(
  ...
  geom_point(
    size = 2,
    mapping = aes(x = FFE, y = Jenkins, color = `Stand Age`, shape = FOREST_TYPE)
  ) +
  coord_cartesian(xlim = c(0, 200), ylim = c(0, 200)) +
  geom_abline(intercept = 0, slope = 1) +
  theme(aspect.ratio = 1) +
...
matching_tree <- fia_trees(fiadb, stand_grow_only) |>
  group_by(STATECD, COUNTYCD, PLOT) |>
...
stand_measured_spatial <- matching_tree |>
  ...
  # Join to get FORTYPE and STDAGE
  left_join(
    nk_matching_cond |> select(STATECD, COUNTYCD, PLOT, INVYR, FORTYPE, STDAGE),
    by=join_by(STATECD, COUNTYCD, PLOT, INVYR)
  )
...
compare_ffe_spatial <- fvs_stand_measured_ffe |>
  ...
  rename(FOREST_TYPE = FORTYPE) |>
  rename(`Stand Age` = STDAGE) |>
  select(StandID, Year, FFE, FOREST_TYPE, `Stand Age`) |>
  left_join(nk_plot_crosswalk, by = join_by(StandID == FVS_STAND_ID)) |>
  left_join(stand_measured_spatial, by = join_by(STATECD, COUNTYCD, PLOT)) |>
  mutate(FIA = CARBON_METRIC) |>
  select(StandID, Year, FFE, FIA, `Stand Age`, FOREST_TYPE)
...
ggplot(
  ...
  geom_point(
    size = 2,
    mapping = aes(x = FFE, y = FIA, color = Year, shape = FOREST_TYPE)
  ) +
  coord_cartesian(xlim = c(0, 200), ylim = c(0, 200)) +
  geom_abline(intercept = 0, slope = 1) +
  theme_bw() +
  ...
  xlab(bquote("FFE Carbon " ~(`Mg C` %.% ha^{-1}))) +
  ylab(bquote("FIA Carbon " ~(`Mg C` %.% ha^{-1}))) 
```
...
ggplot(
  ...
  geom_point(
    size = 2,
    mapping = aes(x = FFE, y = FIA, color = `Stand Age`, shape = FOREST_TYPE)
  ) +
  coord_cartesian(xlim = c(0, 200), ylim = c(0, 200)) +
  geom_abline(intercept = 0, slope = 1) +
  theme_bw() +
...
compare_tpa_spatial <- stand_measured_spatial |>
  select(STATECD, COUNTYCD, PLOT, MEASYEAR, CARBON_METRIC) |>
  ...
  left_join(stand_descr |> select(!MEASYEAR), by = join_by(STATECD, COUNTYCD, PLOT)) |>
...
ggplot(
  ...
  geom_point(
    size = 2,
    mapping = aes(x = Spatial, y = TPA, color = Year, shape = FOREST_TYPE)
  ) +
  coord_cartesian(xlim = c(0, 200), ylim = c(0, 200)) +
  geom_abline(intercept = 0, slope = 1) +
  theme_bw() +
  ...
  xlab(bquote("Spatial Carbon " ~(`Mg C` %.% ha^{-1}))) +
  ylab(bquote("TPA Carbon " ~(`Mg C` %.% ha^{-1}))) 
```
...
ggplot(
  ...
  geom_point(
    size = 2,
    mapping = aes(x = Spatial, y = TPA, color = `Stand Age`, shape = FOREST_TYPE)
  ) +
  coord_cartesian(xlim = c(0, 200), ylim = c(0, 200)) +
  geom_abline(intercept = 0, slope = 1) +
  theme_bw() +
...
stand_measured_ba_fia <- nk_matching_cond |>
  group_by(STATECD, COUNTYCD, PLOT) |>
...
stand_measured_ba_fvs <- nk_ffe_srvy_summary |>
  group_by(StandID) |>
  filter(Year == max(Year)) |>
  ungroup() |>
  ...
  rename(FOREST_TYPE = FORTYPE) |>
  rename(`Stand Age` = STDAGE) |>
  select(StandID, Year, BA_FVS, `Stand Age`, FOREST_TYPE) |>
  left_join(nk_plot_crosswalk, by = join_by(StandID == FVS_STAND_ID)) |>
  select(STAND_CN, BA_FVS, Year, FOREST_TYPE, `Stand Age`)
...
compare_ba_fia_fvs <- stand_measured_ba_fia |>
  left_join(stand_measured_ba_fvs, by = join_by(STAND_CN))
...
ggplot(
  ...
  geom_point(
    size = 2,
    mapping = aes(x = BA_FIA, y = BA_FVS, color = `Stand Age`, shape = FOREST_TYPE)
  ) +
...
compare_ba_meas_proj <- measured |>
  ...
  rename(FOREST_TYPE = FORTYPE) |>
  rename(`Stand Age` = STDAGE) |>
  select(StandID, Year, Measured, Projected, `Stand Age`, FOREST_TYPE)
...
ggplot(
  ...
  geom_point(
    size = 2,
    mapping = aes(x = Measured, y = Projected, color = `Stand Age`, shape = FOREST_TYPE)
  ) +
  coord_cartesian(xlim = c(0, 60), ylim = c(0, 60)) +
  geom_abline(intercept = 0, slope = 1) +
  theme_bw() +
