...
inventory_years <- matching_cond |>
  left_join(numbered_plots, by=join_by(STATECD, UNITCD, COUNTYCD, PLOT, CONDID.x)) |>
...
plot_numbers = numbered_plots |>
  group_by(STATECD, UNITCD, COUNTYCD, PLOT, CONDID.x, .groups = "keep") |>
...
survey_status <- matching_cond |>
  ...
  left_join(plot_numbers, by = join_by(STATECD, UNITCD, COUNTYCD, PLOT, CONDID.x)) |>
  select(
...
ggplot(
  ...
  ggtitle("Plot Survey Status") +
  theme(legend.title = element_blank()) +
  geom_point()

#knitr::kable(survey_status |> filter(PLOT_NUM == 18))
#nik <- matching_cond |>
#  left_join(plot_numbers, by = join_by(STATECD, UNITCD, COUNTYCD, PLOT, CONDID.x)) |>
