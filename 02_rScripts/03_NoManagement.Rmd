...
grown_fvs_stands <- matching_plot |>
  ...
  select(STATECD, UNITCD, COUNTYCD, PLOT, MEASYEAR, INVYR) |>
  left_join(
    nk_plot_ids,
    by = join_by(STATECD, UNITCD, COUNTYCD, PLOT)
  ) |>
  left_join(
    matching_cond |>
      filter(CONDID == 1) |>
      select(STATECD, UNITCD, COUNTYCD, PLOT, INVYR, FORTYPE),
    by = join_by(STATECD, UNITCD, COUNTYCD, PLOT, INVYR)
  ) |>
...
```{r}
fia_carbon <- matching_tree |>
  semi_join(grow_only, by = join_by(STATECD, UNITCD, COUNTYCD, PLOT)) |>
  select(STATECD, UNITCD, COUNTYCD, PLOT, CONDID, INVYR, CARBON_AG) |>
  left_join(
    nk_plot_ids,
    by = join_by(STATECD, UNITCD, COUNTYCD, PLOT)
  )
