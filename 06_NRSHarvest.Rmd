...
```{targets nrs_trees_prsc_harvested, tar_simple = TRUE}
fia_trees_filtered(
    fiadb,
    nrs_plots_prsc_stats_all |> filter(HARVEST == 1),
    filter = \(.data, con) {
...
tree_harvested_but_not_in_pre_harvest <- nrs_trees_prsc_harvested |>
...
```{targets nrshrv_prsc_treelist, tar_simple = TRUE}
fvs_read_output(nrs_harvest_prsc, "FVS_TreeList_East")
```
...
```{targets nrshrv_srvy_treelist, tar_simple = TRUE}
fvs_read_output(nrs_harvest_srvy, "FVS_TreeList_East")
```
...
```{r plot aboveground carbon, eval = FALSE}
ggplot(
    FVS_Carbon_PRSC,
