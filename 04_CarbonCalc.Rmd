...
stand_id_crosswalk <- stand_id_crosswalk |>
  left_join(plt_cns, by = join_by(STATECD, COUNTYCD, PLOT))

DBI::dbDisconnect(fia)

remove(fia, fia_plot, plt_cns)
```
...
```{r nonforest_flag}
matching_cond_nonforest <- matching_cond |>
...
First, determine the first and last years in which at least one plot was measured:
```{r stand_first_last}
stand_first_last <- stand_grow_only |>
  group_by(STATECD, COUNTYCD, PLOT) |>
  mutate(
    FIRSTYEAR_FLAG = MEASYEAR == min(MEASYEAR, na.rm = TRUE),
    STAND_CN = if_else(FIRSTYEAR_FLAG, CN, NA),
    FVS_STAND_ID = if_else(FIRSTYEAR_FLAG, FVS_STAND_ID, NA)
  ) |>
  summarize(
    FIRSTYEAR = min(MEASYEAR, na.rm = TRUE),
    LASTYEAR = max(MEASYEAR, na.rm = TRUE),
    STAND_CN = max(STAND_CN, na.rm = TRUE),
    FVS_STAND_ID = max(FVS_STAND_ID, na.rm = TRUE),
    .groups = "keep"
  ) |>
  ungroup()

knitr::kable(stand_first_last)
```

We also need the background regeneration
```{r background_regen}
nk_regen <- read_csv(
  "data/intermediate/nk_regen.csv", 
  col_types = cols(
    Clearcut = col_integer(), 
    Shelterwood = col_integer(),
    `ITS_Low Retention` = col_integer(), 
    `ITS_High Retention` = col_integer(), 
    Background = col_integer()
  )
)
background_regen <- nk_regen |>
  rename(species = `Species Code`, seedlings = Background) |>
...
keywordfile_section <- function(Title, MgmtId, StandID, StandCN, FirstYear, LastYear, CCMethod) {
  ...
  c(
    ...
    fvs_kwd0("CarbRept"),
    fvs_kwd0("CarbCut"),
    # CarbCalc first param is method:
    # 0 - FFE
    # 1 - Jenkins
    fvs_kwd5("CarbCalc", CCMethod, 1, 0.0425, 9, 11),
    fvs_kwd0("FuelOut"),
    fvs_kwd0("FuelRept"),
    fvs_kwd0("End"), # FMIn
...
Allocate places to gather results:
```{r FVS Outputs}
FVS_Carbon <- NULL
...
```{r FVS_NKFFEProj_NONE.key, include=FALSE}
Title <- "NKFFEProj"
MgmtId <- "NONE"
filename <- paste0("data/fvs/FVS_", Title, "_", MgmtId, ".key")
unlink(filename)
apply(
  stand_first_last |> arrange(FVS_STAND_ID),
  1,
  function(row) {
    write_lines(
      keywordfile_section(
...
```{r fvs_stand_projected_ffe}
# NB: This depends on Title and MgmtId being set correctly!
...
FVS_Carbon <- bind_rows(
  FVS_Carbon,
  tbl(fvs_out_db, 'FVS_Carbon') |>
    collect() |>
    # FVS sometimes inserts duplicate rows? In any case, remove dupes.
    group_by(StandID, Year) |> filter(row_number() == 1) |> ungroup() |>
    mutate(Method = 'Projected', Algorithm = 'FFE')
)
FVS_Summary2_East <- bind_rows(
  FVS_Summary2_East,
  tbl(fvs_out_db, 'FVS_Summary2_East') |>
    collect() |>
    # FVS sometimes inserts duplicate rows? In any case, remove dupes.
    group_by(StandID, Year) |> filter(row_number() == 1) |> ungroup() |>
    mutate(Method = 'Projected', Algorithm = 'FFE')
...
fvs_stand_projected_ffe <- FVS_Carbon |>
  filter(Method == 'Projected' & Algorithm == 'FFE') |>
...
```{r FVS_NKFFEMeas_NONE.key, include=FALSE}
Title <- "NKFFEMeas"
MgmtId <- "NONE"
filename <- paste0("data/fvs/FVS_", Title, "_", MgmtId, ".key")
unlink(filename)
apply(
  stand_grow_only |> arrange(FVS_STAND_ID, MEASYEAR),
  1,
  function(row) {
    write_lines(
      keywordfile_section(
...
FVS_Carbon <- bind_rows(
  ...
  tbl(fvs_out_db, 'FVS_Carbon') |>
    ...
    # FVS sometimes inserts duplicate rows? In any case, remove dupes.
    group_by(StandID, Year) |> filter(row_number() == 1) |> ungroup() |>
    mutate(Method = 'Measured', Algorithm = 'FFE')
)
FVS_Summary2_East <- bind_rows(
  FVS_Summary2_East,
  tbl(fvs_out_db, 'FVS_Summary2_East') |>
...
```{r FVS_NKJenkinsProj_NONE.key, include=FALSE}
Title <- "NKJenkinsProj"
MgmtId <- "NONE"
filename <- paste0("data/fvs/FVS_", Title, "_", MgmtId, ".key")
unlink(filename)
...
fvs_stand_projected_jenkins <- FVS_Carbon |>
  filter(Method == 'Projected' & Algorithm == 'Jenkins') |>
  rename(Aboveground_Live = Aboveground_Total_Live) |>
  rename(Down_Dead = Forest_Down_Dead_Wood) |>
  mutate(Aboveground_Dead = Standing_Dead + Down_Dead) |>
  mutate(Aboveground_Carbon = Aboveground_Live + Aboveground_Dead) |>
...
```{r FVS_NKJenkinsMeas_NONE.key, include=FALSE}
Title <- "NKJenkinsMeas"
MgmtId <- "NONE"
filename <- paste0("data/fvs/FVS_", Title, "_", MgmtId, ".key")
unlink(filename)
...
FVS_Carbon <- bind_rows(
  ...
  tbl(fvs_out_db, 'FVS_Carbon') |>
    ...
    # FVS sometimes inserts duplicate rows? In any case, remove dupes.
    group_by(StandID, Year) |> filter(row_number() == 1) |> ungroup() |>
    mutate(Method = 'Measured', Algorithm = 'Jenkins')
)
FVS_Summary2_East <- bind_rows(
  FVS_Summary2_East,
  tbl(fvs_out_db, 'FVS_Summary2_East') |>
...
fvs_stand_measured_jenkins <- FVS_Carbon |>
  filter(Method == 'Measured' & Algorithm == 'Jenkins') |>
  rename(Aboveground_Live = Aboveground_Total_Live) |>
  rename(Down_Dead = Forest_Down_Dead_Wood) |>
  mutate(Aboveground_Dead = Standing_Dead + Down_Dead) |>
  mutate(Aboveground_Carbon = Aboveground_Live + Aboveground_Dead) |>
...
ggplot(data = compare_ffe) +
  ...
  xlab(bquote("Measured Carbon " ~(Mg %*% ha^{-1}))) +
  ylab(bquote("Projected Carbon " ~(Mg %*% ha^{-1}))) +
  ggtitle("NK Grow-Only Stands Measured vs. Projected Carbon (FFE)") +
...
```{r jenkins_projected_vs_measured}
...
```{r compare_ffe_jenkins}
stand_descr <- matching_cond |>
  select(PLT_CN, STDAGE, FORTYPE) |>
  full_join(stand_id_crosswalk, by = join_by(PLT_CN))
...
ggplot(
  ...
  ylab(bquote("Jenkins Carbon " ~(Mg %*% ha^{-1}))) 
```
...
```{r plot_measured_vs_projected_ba}
ggplot(
