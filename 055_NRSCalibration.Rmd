...
Show us the stats
```{r calibrated vs measured linear regression, eval = FALSE}
summary(lm(
  Projected_Carbon_Flux ~ Measured_Carbon_Flux,
  data = nrsgro_calb_proj_vs_meas |> filter(Projection_Years > 0)
))
```
Compare to lm without calibration:
```
Residuals:
    Min      1Q  Median      3Q     Max 
-4.5569 -0.4120  0.0535  0.4510  4.2553 

Coefficients:
                     Estimate Std. Error t value Pr(>|t|)    
(Intercept)          -1.79345    0.01981  -90.51   <2e-16 ***
Measured_Carbon_Flux  0.17030    0.01221   13.95   <2e-16 ***
```
