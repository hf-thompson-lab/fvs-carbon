...
multiplier (385/4=95.25, since there are 4x6'radius microplots), rather than
the plot multiplier (5). There is a history
...
tmp_top_species <- tmp_by_species |>
  ...
  filter(BA_Frac > 0.1) # Use 0.075 to catch Quercus velutina, 0.07 for Tsuga canadensis
...
tmp_trees |>
  ...
  scale_y_continuous(transform = "log10") +
  theme_bw() +
  ggtitle("CFI Plot Species Composition (1970)") +
  xlab(bquote("Basal Area" ~ (m^2 %.% hectare^-1))) +
  ylab(bquote("Density" ~ (stems %.% hectare^-1))) +
  facet_wrap("Species")
```
...
There are some species in CFI that FVS doesn't handle; make a
substitution table.
...
tblDWSPCFITreesComplete |>
  ...
  left_join(species_crosswalk, by = join_by(SpeciesCode == SPCD)) |>
  filter(is.na(FVS_SPCD))
```

Substitutions:
- Populus spp. / unknown cottonwood or poplar
  - Replace with Populus grandidentata, which is the most common Populus
- Carya cordiformis / bitternut hickory
  - Carya glabra - pignut hickory; very similar
- Castanea dentata / American chestnut
  - FVS does not handle any chestnuts, and these will never get big
- Sorbus americana / American mountain-ash
  - FVS does not handle any mountain-ash, and these will never get big
- Acer platanoides / Norway maple
  - Sugar saccharum / sugar maple; a hard maple FVS does handle
- Phellodendron amurense / Amur corktree
  - No other species in this genus
- Unknown tree
...
# CFI-Specific Modeled BAI Residual Error
...
```{r feature selection, eval = FALSE, include=FALSE}
cfigro_training_features <- c(
  "ASPECT", "ELEV", "LAT", "SLOPE",
  "Acer_rubrum", "Pinus_strobus", "Quercus_rubra",
  "Projected_BA", "Projected_Tph"
)
...
# for BAI, ignore Carbon Flux
cfigro_bai_task$select(cfigro_training_features)
...
cfigro_bai_fs_task$result |>
  pivot_longer(cols = -any_of(c("Carbon_Flux_Residual", "features", "n_features", "regr.rmse"))) |>
  filter(value) |>
  pull(name)
```
[1] "Acer_rubrum" "LAT" "Pinus_strobus" "Projected_BA" "Projected_Tph" "Quercus_rubra", "SLOPE"

It selected the two projected quantities, and the three species selectors, and LAT.

## Hyperparameters

```{r optimize hyperparameters, eval=FALSE, include=FALSE}
cfigro_bai_task$select(
  c("Acer_rubrum", "LAT", "Pinus_strobus", "Projected_BA", "Projected_Tph", "Quercus_rubra", "SLOPE")
)

cfigro_bai_lrn_regr_ranger <- lrn(
  "regr.ranger",
...
cfigro_bai_hp_task <- tune(
  tuner = tnr("mbo"),
  task = cfigro_bai_task,
  learner = cfigro_bai_lrn_regr_ranger,
  resampling = rsmp("repeated_cv", repeats = 10, folds = 10),
  measures = msr("regr.rmse"),
  terminator = trm("stagnation", threshold = 0.1)
)

unlist(cfigro_bai_hp_task$result_x_domain)
```

```
> unlist(cfigro_bai_hp_task$result_x_domain)
   importance     max.depth min.node.size          mtry     num.trees 
"permutation"          "16"           "5"           "3"          "22"  

> unlist(cfigro_bai_hp_task$result_x_domain)
   importance     max.depth min.node.size          mtry     num.trees 
"permutation"          "14"          "11"           "3"          "49" 

> unlist(cfigro_bai_hp_task$result_x_domain)
   importance     max.depth min.node.size          mtry     num.trees 
"permutation"          "32"           "5"           "2"          "19"

> unlist(cfigro_bai_hp_task$result_x_domain)
   importance     max.depth min.node.size          mtry     num.trees 
"permutation"          "13"           "5"           "3"          "25" 
...
The tuning is somewaht variable; the first one seems relatively
representative.
...
autoplot(cfigro_bai_model$predict(cfigro_bai_task)) +
  stat_poly_eq(use_label("eq", "R2"), formula = x ~ y, color = "blue") +
...
cfigro_training_data |>
  mutate(
    Adjusted_BAI = Projected_BAI - cfigro_bai_model$predict(cfigro_bai_task)$response
  ) |>
  select(
    StandID,
    Adjusted_BAI,
...
# CFI-Specific Modeled Aboveground Live Carbon Flux Residual Error

How does carbon flux residual look compared to BAI residual?

```{r aglcf vs bai, eval = FALSE}
cfigro_training_data |>
  ggplot(aes(BAI_Residual, Carbon_Flux_Residual)) +
  geom_point() +
  stat_poly_line(
    formula = y ~ x,
    fullrange = TRUE,
...
They are very highly correlated, so predicting above ground live carbon flux
residual should be similar to predicting basal area increment residual.

## Train & Predict

```{targets cfigro_aglcf_model, tar_simple = TRUE}
...
```{r plot cfigro_aglcf_model, eval = FALSE}
autoplot(cfigro_aglcf_model$predict(cfigro_aglcf_task)) +
  stat_poly_eq(use_label("eq", "R2"), formula = x ~ y, color = "blue") +
  xlab("Predicted Aboveground Live Carbon Flux Residual") +
  ylab("Actual Aboveground Live Carbon Flux Residual") +
  ggtitle("Modeled FVS Aboveground Live Carbon Flux Residual on CFI Grow-Only Plots")
```

```{r plot cfigro_aglcf_model density, eval = FALSE}
cfigro_training_data |>
  mutate(Adjusted_Carbon_Flux = Projected_Carbon_Flux - cfigro_aglcf_model$predict(cfigro_aglcf_task)$response) |>
  select(
    StandID,
    `FVS Adjusted` = Adjusted_Carbon_Flux,
    FVS = Projected_Carbon_Flux,
    Observed = Measured_Carbon_Flux
  ) |>
  pivot_longer(
    cols = c("FVS Adjusted", "FVS", "Observed"),
    names_to = "Series",
    values_to = "Carbon_Flux"
  ) |>
  mutate(
    Series = ordered(Series, levels = c("Observed", "FVS", "FVS Adjusted"))
  ) |>
  ggplot(aes(Carbon_Flux, fill = Series)) +
  geom_density(alpha = 0.5) +
  geom_vline(xintercept = 0, color = "black", linewidth = 0.25) +
  theme_bw() +
  xlab(bquote("Aboveground Live Carbon Flux" ~ (`Mg C` %.% ha^-1 %.% year^-1))) +
  ylab("Density") +
  scale_fill_brewer(palette="Set1") +
  ggtitle("Modeled vs. Observed AGL Carbon Flux")
```

## Error Distribution

```{r plot cfigro_aglcf_model density2, eval = FALSE}
cfigro_training_data |>
  mutate(
    Modeled_Residual = cfigro_aglcf_model$predict(cfigro_aglcf_task)$response
  ) |>
  select(
    StandID,
    Observed = Carbon_Flux_Residual,
    Modeled = Modeled_Residual
  ) |>
  pivot_longer(
    cols = c("Observed", "Modeled"),
    names_to = "Series",
    values_to = "Carbon_Flux_Residual"
  ) |>
  mutate(
    Series = ordered(Series, levels = c("Observed", "Modeled"))
  ) |>
  ggplot(aes(Carbon_Flux_Residual, fill = Series)) +
  geom_density(alpha = 0.5) +
  geom_vline(xintercept = 0, color = "black", linewidth = 0.25) +
  theme_bw() +
  ...
  ylab("Density") +
  scale_fill_brewer(palette="Set1") +
  ggtitle("Modeled vs Observed AGL Carbon Flux Residual")
```

# Plots as Sample

When looking at CFICOP, we treat the plots as a sample from the landscape
(the forest), and compute a 95% confidence interval for the population
mean.

Let's do the same thing here.

- Treat the 31 plots as a sample (by area?)
...
from above, estimated 56816 forested acres; we have 31 * 0.2 = 6.2 acres,
so 0.011%

So, let's look at distribution of biomass in our 277 plot sample:

```{r population mean, eval = FALSE}
hectares <- conv_unit(56816, "acre", "hectare")

population_mean <- cfigro_none_proj_vs_meas |>
  group_by(Year) |>
  summarize(
    measured_carbon_mean = mean(Measured_Carbon),
    measured_carbon_sd = sd(Measured_Carbon),
  ...
  ungroup() |>
  mutate(
    # since we multiply by hectares later, convert sd to ci via 1.96 / sqrt(n)
    measured_carbon_min = measured_carbon_mean - 1.96 * measured_carbon_sd / sqrt(n),
    measured_carbon_max = measured_carbon_mean + 1.96 * measured_carbon_sd / sqrt(n),
    projected_carbon_min = projected_carbon_mean - 1.96 * projected_carbon_sd / sqrt(n),
    projected_carbon_max = projected_carbon_mean + 1.96 * projected_carbon_sd / sqrt(n)  ) |>
  select(
    Year,
    measured_carbon_mean, measured_carbon_min, measured_carbon_max,
    projected_carbon_mean, projected_carbon_min, projected_carbon_max
  ) |>
  pivot_longer(-Year) |>
  mutate(
    value = value * hectares,
    group = if_else(grepl("measured", name), "measured", "projected"),
...
population_mean |>
  mutate(group = ordered(group, levels = c("measured", "projected"))) |>
  ggplot(aes(Year, mean)) +
  geom_ribbon(aes(ymin = min, ymax = max, fill = group), alpha = 0.25) +
  geom_line(aes(color = group)) +
  coord_cartesian(ylim = c(0, NA)) +
  theme_bw() +
  ggtitle("Projected vs. Measured Total Carbon, 0.01% sample") +
  ylab(bquote("Total Carbon" ~(`Mg C`))) +
  scale_color_discrete(name = "Estimated\nPopulation\nMean") +
  scale_fill_discrete(name = "95% CI")
```

```{r adjusted_population_mean, eval = FALSE}
# These definitions need to match what's used in the target to build the model
cfigro_aglcf_task <- as_task_regr(
  cfigro_training_data,
  target = "Carbon_Flux_Residual"
)
...
adjusted_population_mean <- cfigro_training_data |>
  mutate(
    Modeled_Residual = cfigro_aglcf_model$predict(cfigro_aglcf_task)$response,
    Adjusted_Carbon_Flux = Projected_Carbon_Flux - Modeled_Residual
  ) |>
  select(
    StandID,
    Adjusted_Carbon_Flux
  ) |>
  cross_join(
    cfigro_none_proj_vs_meas |> distinct(Year)
  ) |>
  left_join(
    cfigro_none_proj_vs_meas,
  ...
  mutate(
    Adjusted_Carbon = Starting_Carbon + (Adjusted_Carbon_Flux * (Year - 1970))
  ) |>
  select(
    StandID, Year, Measured_Carbon, Projected_Carbon, Adjusted_Carbon
  ) |>
  group_by(Year) |>
  summarize(
    measured_carbon_mean = mean(Measured_Carbon),
  ...
  mutate(
    ...
    projected_carbon_min = projected_carbon_mean - 1.96 * projected_carbon_sd / sqrt(n),
    projected_carbon_max = projected_carbon_mean + 1.96 * projected_carbon_sd / sqrt(n),
    adjusted_carbon_min = adjusted_carbon_mean - 1.96 * adjusted_carbon_sd / sqrt(n),
    adjusted_carbon_max = adjusted_carbon_mean + 1.96 * adjusted_carbon_sd / sqrt(n)
    ) |>
  select(
    Year,
    measured_carbon_mean, measured_carbon_min, measured_carbon_max,
    projected_carbon_mean, projected_carbon_min, projected_carbon_max,
    adjusted_carbon_mean, adjusted_carbon_min, adjusted_carbon_max
  ) |>
  pivot_longer(-Year) |>
  mutate(
    value = value * hectares,
    group = if_else(
      grepl("measured", name), "measured",
      if_else(grepl("adjusted", name), "adjusted",
              "projected")
    ),
    kind  = if_else(
      grepl("_min", name), "min",
      if_else(grepl("_max", name), "max", "mean")
    )
  ) |>
  pivot_wider(id_cols = c("group", "Year"), names_from = "kind")
...
```{r plot adjusted population mean, eval = FALSE}
adjusted_population_mean |>
  mutate(group = ordered(group, levels = c("measured", "projected", "adjusted"))) |>
  ggplot(aes(Year, mean)) +
  geom_ribbon(aes(ymin = min, ymax = max, fill = group), alpha = 0.25) +
  geom_line(aes(color = group)) +
  coord_cartesian(ylim = c(0, NA)) +
