...
Aside: does the BA calculated from trees line up with BALIVE from FIA?
```{r ba vs balive, eval = FALSE}
nrshrv_prsc_treelist |>
  group_by(StandID) |>
  filter(Year == min(Year)) |>
  summarize(
    StartingBA = sum(TPA * pi * (DBH / 12 / 2)^2, na.rm = TRUE),
...
trees_in_harvest_inventory <- fia_trees_filtered(fiadb, harvested_plots, \(.data, con) {
  .data |>
    select(
      CN, STATECD, COUNTYCD, PLOT, INVYR,
...
pre_harvest_bai_trees <- trees_in_harvest_inventory |>
  filter(STATUSCD %in% c(1, 3)) |> # live or harvested, not dead or no status
  left_join(
    nrshrv_plot_stats |>
      filter(HARVEST == 1) |>
...
But even FVS input is off from FIA BALIVE, so ignore for now.
