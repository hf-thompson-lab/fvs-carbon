...
projected_annual_growth <- starting_trees |>
...
for (partition in 1:8) {
  ...
  fvs_out_db <- DBI::dbConnect(
    RSQLite::SQLite(),
    paste0('data/fvs/FVS_NRSAnnual_NONE_', partition, '.db')
  )
  fvs_carbon_tbl = tbl(fvs_out_db, 'FVS_Carbon') |>
    mutate(Partition = partition)
  fvs_summary2_east_tbl = tbl(fvs_out_db, 'FVS_Summary2_East') |>
  ...
  FVS_Carbon_ANNL <- bind_rows(FVS_Carbon_ANNL, fvs_carbon_tbl |> collect())
  FVS_Summary2_East_ANNL <- bind_rows(FVS_Summary2_East_ANNL, fvs_summary2_east_tbl |> collect())
...
ggplot(
  ...
  ylab(bquote("1-Year Timestep Annual Carbon Flux" ~ (Mg %*% ha^-1))) +
  xlab(bquote("10-Year Timestep Annual Carbon Flux" ~ (Mg %*% ha^-1)))
```
...
ggplot(
  ...
  ggtitle(bquote("Projected vs. Measured Carbon Flux")) +
