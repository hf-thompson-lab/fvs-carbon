...
> Nik is here
> take the 1980 pulse (delta in 6" fron 1970 - 1980?) and turn
> those into extra trees in 1970? 4" trees? 2/3 the height?
> 2/3 the DBH? (2/3 because 4 is 2/3 of 6)
> also, i feel like i was looking at a spot in harvest where we're
> looking at fvs trees without summing tpa
> Also also, instead of ThinDBH, try ThinBTA - thin from below to
> a TPA target.
> - DBH range - everything
> - TPA target - what's left on the plot
> - Efficiency - calculate ratio of initial to residual tpa in each
>   diameter class, take the max? mean? median?
> ThinBTA doesn't allow species selection; ThinDBH does, can
> maybe use ThinDBH with Cuteff to achieve the same goal? Note that
> ThinDBH ignores CutEFF if target TPA or BA is specified.
> ThinQFA also allows species to be specified; can thin to TPA,
>    specified in supplemental record.
...
Rather than derive a principled way to turn 1980 ingrowth into
1970 saplings, we can represent the 1970 sapling population by
actual 1980 ingrowth. By 1980, the 1970 seedling should have
created a sapling pool that FVS can track, so we don't need to
do anything beyond 1980.
So let's add 1980 establishment as a 1-off.
We use actual trees and height when available.
...
FVS provides the THINDBH keyword to schedule thinning from throughout
a dbh range for any or all species to a residual basal area or trees
per acre target.
...
FVS will have in the stand at any given time, so we can't compute
the residual number of trees that would cause the right number to be
removed.

We could also do this by telling FVS the fraction of trees to
remove; we know how many were harvested and when, but we don't know
how many trees were on the landscape at the time of harvest, so we
can't compute the correct fraction. By looking at cficop_dflt_treelist
and cficop_none_treelist, we can get a pretty good idea of what FVS
thinks is there.
...
- Trees on the landscape grow at different rates than in FVS;
  it's too hard to try to hit 2" diameter classes, so we use
  much wider 6" diameter classes to try to catch more trees.
...
FVS really doesn't like to do harvest. Let's load up the UI and see if we can
convince it.

### FvS_StandInit and FVS_PlotInit

From DBSUserGuide.pdf, Table 3.0.1, we learn the following about table structure:

* Stand_CN - optional, but used by queries, so we'll include it
* Stand_ID - required
* StandPlot_ID - optional; plot identification code;
             required by the user interface when populating stand lists from
             the plot initiation table (PlotInit)
* Variant  - required; set to "ne"
* Inv_Year - required; set to 1970
* Latitude - optional
* Longitude - optional
* Location - recommended; nearest national forest;
             takes precedence over Region, forest, District, and Compartment
...
* Brk_DBH  - required
* Num_Plots - recommended; if 0, the number of plots in the stand is determined
              by counting the numbers [sic] of unique plot identification
              codes on the tree record data

All other fields are optional.

### FVS_TreeInit

From DBSUserGuide.pdf, Table 3.0.2, we learn the following about table structure:
...
* Tree_ID  - optional
* Tree_Count - recommended - the number of trees represented by the tree record;
             if left blank FVS assumes the tree record represents a single sample
             tree
* Species  - required
* DBH or Diameter - required
* Ht       - recommended; total height in feet
* CrRatio  - recommended; values 1-9 are read as crown ratio codes; 10-99 as percent
...
And it's not just trees of species "Other".
...
observed_trees <- qryDWSPCFIPlotVisitTreeDetail |>
  ...
  filter(cfi_status_live(VisitTreeStatusCode)) |>
  mutate(
    diameter_class = floor(VisitTreeDIAM / 2) * 2,
    Scenario = "Observed",
...
Let's pick an example; MasterPlotID 10153, STAND_CN 119700153, we
we request to harvest white pine (SPCD==129) in diameter class 6-8"
down to 60 TPA in 1993; this harvest gets canceled.

```{r, eval = FALSE}
cficop_hvst_tpa |> filter(STAND_CN == "119700153") |> arrange(SPCD, DBH_MIN)
```

Let's look at pre-harvest number of trees
```{r pre-harvest trees, eval = FALSE}
stand_10153_trees <- fvs_read_output(cficop_hvst, "FVS_TreeList_East") |>
  filter(StandID == "10153") |>
  filter(random_seed == min(random_seed))
```

```{r, eval = FALSE}
tmp_modeled <- stand_10153_trees |>
  filter(SpeciesFIA == 129) |>
  filter(1970 <= Year & Year <= 2000) |>
  mutate(
    diameter_class = floor(DBH / 2) * 2
  ) |>
  group_by(Year, diameter_class) |>
  summarize(TPA = sum(TPA), .groups = "drop") |>
  mutate(series = "Modeled") |>
  complete(
    Year, diameter_class,
    fill = list(TPA = 0, series = "Modeled")
  )

tmp_observed <- cfiabp_trees |>
  filter(MasterPlotID == "10153") |>
  filter(USFSCode == 129) |>
  filter(VisitTreeStatusCode == 1) |>
  filter(1970 <= VisitCycle & VisitCycle <= 2000) |>
  mutate(
    Year = VisitCycle,
    diameter_class = 2 * floor(conv_unit(dbhcm, "cm", "in") / 2)
  ) |>
  group_by(Year, diameter_class) |>
  summarize(TPA = n() * 5, .groups = "drop") |>
  mutate(series = "Observed") |>
  complete(
    Year, diameter_class,
    fill = list(TPA = 0, series = "Observed")
  )

tmp_modeled |>
  union_all(tmp_observed) |>
  filter(diameter_class > 2) |>
  mutate(
    Year = ordered(Year),
    diameter_class = ordered(diameter_class)
  ) |>
  ggplot(aes(series, TPA, fill = diameter_class, group = diameter_class)) +
  geom_col(position = "dodge") +
  facet_wrap("Year") +
  theme_bw() +
  labs(
    title = "Trees per acre by diameter class in plot 10153",
    fill = "Diameter Class"
  ) +
  ylab("TPA") +
  xlab(NULL)
...
Here we can see that in 1990, there were not enough trees in the 6-8" diameter
class in 1990.

What's the rolling establishment for this plot?
```{r, eval = FALSE}
tmp_stand_cn <- cficop_plot_sibling |>
  filter(MasterPlotID == "10153") |>
  left_join(
    cfigro_plot |>
      select(MasterPlotID = STAND_ID, STAND_CN),
    by = join_by(MasterPlotID)
  ) |>
  pull(STAND_CN)

cfigro_estab |>
  filter(STAND_CN == tmp_stand_cn)
  
```

What about the 1980 pulse?
...
Ugh; stand 119700442 was harvested twice between 1970 and 1980.
Let's ignore it for now; probably the right thing to do is to
consolidate into a single harvest - the earlier one to give
regeneration more time, or the later one to give regeneration
less time.

```{r hvst vs srvy remainders, eval = FALSE}
measured_remainders <- cficop_hvst_tpa |>
  mutate(SPCD = as.numeric(SPCD)) |>
  left_join(
    species_crosswalk |> select(SPCD, GENUS),
    by = join_by(SPCD)
  ) |>
  group_by(GENUS) |>
  summarize(
    Measured_Trees = sum(TPA),
    .groups = "drop"
  )
...
measured_remainders |>
  ...
  #facet_wrap("name") +
  #scale_y_continuous(transform = "log10") +
  ggtitle("Quabbin CFI Trees Remaining") +
  xlab("Genus") +
  theme(axis.text.x = element_text(angle = -30, hjust = 0, vjust = 1)) +
  ylab(bquote("Trees remaining" ~ (trees)))
```
...
cficop_none_carbon |>
  ...
  mutate(name = ordered(
    name,
    levels = c("Standard", "Calibrated", "Best Estimate", "Reference", "Measured")
  )) |>
  ggplot(aes(Year, mean, color = name)) +
  with_blur(
    geom_ribbon(aes(ymin = ymin, ymax = ymax, fill = name), alpha = 0.25, color = NA),
    sigma = 3
...
#  scale_y_continuous(limits = c(0, NA)) +
  ggtitle("Carbon in Quabbin CFI plots") +
  xlab("Year") +
...
cficop_none_carbon |>
  ...
  select(Scenario = name, `AGLC` = mean, ci)
```
...
```{r none vs dflt vs srvy alt, eval = FALSE}
