...
starting_ba <- nrshrv_prsc_treelist |>
  ...
  summarize(
    StartingBA = sum(TPA * pi * (DBH / 12) / 2, na.rm = TRUE),
    .groups = "keep"
  ) |>
  ungroup() |>
  mutate(
...
harvested_ba <- nrshrv_prsc_cutlist |>
  group_by(StandID) |>
  summarize(
    HarvestedBA = sum(TPA * pi * (DBH / 12) / 2, na.rm = TRUE),
    .groups = "keep"
  ) |>
  ungroup() |>
  mutate(
...
harvest_summary <- harvest_summary |>
  mutate(
    StandBA = ordered(case_when(
      StartingBA < 90 ~ "< 90",
      StartingBA < 115 ~ "90 - 115",
      StartingBA < 140 ~ "115 - 140",
      .default = ">= 140"
    ), levels = c("< 90", "90 - 115", "115 - 140", ">= 140")),
    HarvestIntensity = ordered(case_when(
      HarvestFraction < 0.1 ~ "< 0.1",
      HarvestFraction < 0.2 ~ "0.1 - 0.2",
      HarvestFraction < 0.33 ~ "0.2 - 0.3",
      .default = ">= 0.3"
