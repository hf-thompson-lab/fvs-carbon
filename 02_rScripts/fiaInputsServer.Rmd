...
#TODO eventually pull concatPlot 25_13_14 and match landis output (drop HH) before saving in fvs format.  5 spp max, and no rare species or rare species for both
...
# join plotCondft here
 alltreesPrep <- alltrees %>%
   ...
   dplyr::select(STATE,STATECD,UNITCD,COUNTYCD,PLOT,PLT_CN,SUBP,INVYR,DIA,CARBON_AG,STATUSCD,REGIONAL_DRYBIOT,TREE,SPCD) %>% #,TREE,SPCD,DRYBIO_BOLE,DRYBIO_STUMP,DRYBIO_AG,TREE,SPCD,
...
```{r full plot prep}
 plotout <- trees1short %>% 
   filter(STATUSCD==1) %>% 
   left_join(plotCondft) %>% #if full plot level, plotContft
   filter(plotAreaForest>= 672) %>% #672.44
   ungroup() %>% 
   mutate(treeAGCMg=ifelse(DIA>=5,AGCMg,NA),
          sapAGCMg=ifelse(DIA<5,AGCMg,NA),
          treeBAm2=ifelse(DIA>=5,BAm2,NA),
          sapBAm2=ifelse(DIA<5,BAm2,NA)) %>% 
  group_by(concatPlot,PLT_CN,topSTDAGE) %>% 
  #group_by(concatPlot,INVYR,PLT_CN,topSTDAGE) %>% #if full plot level
  summarise(plotTreeBAm2=sum(treeBAm2,na.rm=TRUE),
            plotSapBAm2=sum(sapBAm2,na.rm=TRUE),
            plotTreeAGCMg=sum(treeAGCMg,na.rm=TRUE),
   ...
   ungroup() %>% 
   mutate(plotTreeBAm2ha=plotTreeBAm2*(10000/672.44), #10000/672.44 if full plot
          plotSapBAm2ha=plotSapBAm2*(10000/(4*13.5)),
          plotTreeAGCMgha=plotTreeAGCMg*(10000/672.44),
          plotSapAGCMgha=plotSapAGCMg*(10000/(4*13.5)),
          plotSapBAm2haV2=plotSapBAm2*(74.965282)*2.47,
          plotSapAGCMghaV2=plotSapAGCMg*(74.965282)*2.47) %>% 
   # group_by(UNIQUESUBP) %>%   mutate(tBAm2ha=sum(plotTreeBAm2ha,plotSapBAm2ha,na.rm=TRUE),tAGCMgha=sum(plotTreeAGCMgha,plotSapAGCMgha,na.rm=TRUE)) %>%
   # ungroup() %>% 
   # mutate(tBAftac=tBAm2ha*4.356) %>% 
  #   filter(PLT_CN==247057536010661) %>%  
    filter(PLT_CN %in% possibles$PLT_CN)
...
possibleIn <- possibles %>% 
  ...
  ungroup() 
#
possiblecw <- possibleIn %>% dplyr::select(PLT_CN,concatPlot,STAND_ID) %>% mutate(StandID=STAND_ID) %>% distinct()
...
#for Ben
```{r compare outputs temp}
#
#you can just call fvsout in below
fvsout <- readRDS("Y:/REU2023/FVS/03_outputsmbbV3_20230725.Rds") %>% 
  mutate(StandID=as.character(StandID))  %>% 
  left_join(possiblecw) %>% 
  left_join(plotout %>% dplyr::select(concatPlot,PLT_CN,topSTDAGE,plotTreeAGCMgha ,plotSapAGCMgha)) %>% 
  mutate(tFIAAGCMgha=plotTreeAGCMgha+plotSapAGCMgha,
         pctdif=round((AGMgha-tFIAAGCMgha)/tFIAAGCMgha*100) ) %>% 
  arrange(abs(pctdif)) %>% 
  dplyr::select(concatPlot,PLT_CN,StandID,pctdif,everything())
fvsout
#saveRDS(fvsout,"Y:/REU2023/FVS/03_outputs/mbbV3_fvsfiaSummaryForCompare.Rds")

#look at:
# n subplots
# saplings/trees
# status codes (STATUSCD==1 is live)
fvsout <- readRDS("Y:/REU2023/FVS/03_outputs/mbbV3_fvsfiaSummaryForCompare.Rds")
treesPrep1
plotCondft
# fvs input
"Y:\REU2023\FVS\01_processedInputData\MA_plots_mbbV3.xlsx"
#
```
