...
write_rds(plot_grow_only, 'data/intermediate/plot_grow_only.rds')
...
ingrowth <- tbl(fia, 'TREE_GRM_COMPONENT') |>
  ...
  ingrowth_filter() |>
  select(
    TRE_CN, PREV_TRE_CN, PLT_CN, STATECD,
...
fia_tree_cols <- function(.data) {
  .data |>
    select(
      ...
      INVYR, STATECD, UNITCD, PLOT, SUBP, TREE, CONDID,
...
#  select(TRE_CN) |>
  select(PREV_TRE_CN) |> rename(TRE_CN = PREV_TRE_CN) |>
  filter(!is.na(TRE_CN)) |>
  rename(CN = TRE_CN) |>
  distinct()
...
while (nrow(prev_tre_cns) > 0) {
  ...
  more_prev_trees <- fia_tree |>
    semi_join(prev_tre_cns, by = join_by(CN), copy = TRUE) |>
    fia_tree_cols() |>
    collect()
  prev_tree <- rbind(prev_tree, more_prev_trees) |>
    distinct()
  prev_tre_cns <- more_prev_trees |>
    select(PREV_TRE_CN) |>
...
dbDisconnect(fia)
remove(fia, plt_cns, fia_tree, prev_tre_cns)
```
...
For the remaining 29,373 trees, we will need to back-project. TREE_GRM_COMPONENT
provides annualized growth rates for both DIA and HT.

> TODO Nik: Do these growth rates make sense? We can validate using our 705 trees.
...
keywordfile_section <- function(Title, MgmtId, StandID, StandCN, FirstYear, LastYear, Partition) {
  ...
  LastYear <- as.integer(LastYear)#  + 1 #LastYear + 1 is the cycle boundary
  NumCycles <- as.integer((LastYear - FirstYear) / 10) + 1
  ShortCycle <- (LastYear - FirstYear) %% 10
  TimeConfig <- NULL
  if (NumCycles == 0) {
    # Produce a single cycle of the necessary duration
    TimeConfig <- c(
      fvs_kwd1("InvYear", FirstYear),
      fvs_kwd2("TimeInt", 0, ShortCycle),
      fvs_kwd1("NumCycle", 1)
    )
  } else if (ShortCycle > 0) {
...
There are 66 missing plots; where are they?

```{r missing_standinit}
missing_stands <- matched_carbon_tmp |>
  filter(is.na(Starting_Carbon)) |>
  select(STAND_ID, STAND_CN, STATECD, COUNTYCD, PLOT, INVYR)
fia <- DBI::dbConnect(RSQLite::SQLite(), 'data/raw/SQLite_FIADB_ENTIRE.db')
missing_standinit <- tbl(fia, 'FVS_StandInit_Plot') |>
  semi_join(missing_stands, by = join_by(STAND_CN), copy = TRUE) |>
  collect()
...
OK, they're not in FVS_StandInit_Plot. Do they exist at all?

```{r missing_plot}
fia <- DBI::dbConnect(RSQLite::SQLite(), 'data/raw/SQLite_FIADB_ENTIRE.db')
missing_plot <- tbl(fia, 'PLOT') |>
  semi_join(missing_stands, by = join_by(CN == STAND_CN), copy = TRUE) |>
  collect()
...
They're there; all are in STATECD=39, various years counties and plots, but 
all are MODIFIED_BY=TROFFE
so I wonder if there's a modified version of the stand that's getting picked up.
Let's see if we can find them by STAND_ID
```{r missing_standinit_by_stand_id}
fia <- DBI::dbConnect(RSQLite::SQLite(), 'data/raw/SQLite_FIADB_ENTIRE.db')
...
missing_standinit_by_stand_id <- tbl(fia, 'FVS_StandInit_Plot') |>
  semi_join(missing_stands, by = join_by(STAND_ID), copy = TRUE) |>
  collect()
...
Let's look for just the first one:
```{r first_missing}
first_missing <- missing_stands |> head(1)
first_missing
```

Any other stands present for this one?
```{r missing_plot_by_location}
fia <- DBI::dbConnect(RSQLite::SQLite(), 'data/raw/SQLite_FIADB_ENTIRE.db')
#first_missing_plot <- tbl(fia, 'PLOT') |>
#  semi_join(first_missing, by = join_by(STATECD, COUNTYCD, PLOT), copy = TRUE)
missing_stand_by_location <- tbl(fia, 'FVS_StandInit_Plot') |>
  filter(STAND_CN == '109827807010661') |>
  collect()
...
Anything interesting on the associated survey?
```{r missing_survey}
fia <- DBI::dbConnect(RSQLite::SQLite(), 'data/raw/SQLite_FIADB_ENTIRE.db')
missing_survey <- tbl(fia, 'SURVEY') |>
  filter(CN == '76387293010538') |>
  collect()
dbDisconnect(fia)
remove(fia)
```
...
Sent a note to FIA asking for help.
