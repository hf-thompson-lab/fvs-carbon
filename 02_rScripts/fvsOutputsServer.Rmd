...
runName <- "MA_Run2"  #"tpa1000_initiation"  #tpa and then defining run
cOut <- read_excel(paste0(outputDir,"/",runName,"_FVSoutput.xlsx"),sheet="FVS_Carbon") %>% 
  dplyr::select(CaseID,StandID,Year,abgCMgha=Aboveground_Total_Live,Belowground_Live,Standing_Dead,Belowground_Dead,Forest_Down_Dead_Wood)
bOut <- read_excel(paste0(outputDir,"/", runName,"_FVSoutput.xlsx"),sheet="FVS_Summary2_East") %>% 
...
#tree level
tOut <- read_excel(paste0(outputDir,"/",runName,"_FVSoutput.xlsx"),sheet="FVS_TreeList_East") 
...
#
# open multiple files
defaultFVSdirectory <- paste0(outputDir,"/")
...
lfcs <- list.files(defaultFVSdirectory,pattern="FVSoutput.xlsx")
setwd(defaultFVSdirectory); fvsout <- map_dfr(.x=set_names(lfcs),
