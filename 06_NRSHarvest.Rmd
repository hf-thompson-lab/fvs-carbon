...
harvest_summary <- harvest_summary |>
  mutate(
    ...
    HarvestIntensity = ordered(case_when(
      ...
      HarvestFraction < 0.35 ~ "0.20 - 0.35",
      HarvestFraction < 0.55 ~ "0.35 - 0.55",
      .default = ">= 0.55"
    ), levels = c("< 0.20", "0.20 - 0.35", "0.35 - 0.55", ">= 0.55"))
  )
```
...
nrshrv_prsc_carbon |>
  group_by(StandID) |>
  arrange(Year) |>
  mutate(MeasNum = factor(row_number())) |>
  ungroup() |>
  ggplot(aes(MeasNum, YearsSinceHarvest)) +
  geom_boxplot()
```

Which makes it look like we should use year breakpoints of c(-10, 0, 10, 20),
calling out 0 as a special case, so 10<0, 0, 0<10, 10<20.

```{r time buckets, eval = FALSE}
nrshrv_prsc_carbon |>
  mutate(
    TimeStep = factor(case_when(
      YearsSinceHarvest < 0 ~ "-10 - -1",
      YearsSinceHarvest == 0 ~ "0",
      YearsSinceHarvest < 10 ~ "1 - 10",
      .default = "10 - 20"
    ))
  ) |>
...
nrshrv_srvy_carbon |>
  mutate(
    TimeStep = ordered(case_when(
      YearsSinceHarvest < 0 ~ "-5 - 0",
      YearsSinceHarvest < 5 ~ "0 - 5",
      YearsSinceHarvest < 10 ~ "5 - 10",
      YearsSinceHarvest < 15 ~ "10 - 15",
      .default = "15 - 20"
    ), levels = c("-5 - 0", "0 - 5", "5 - 10", "10 - 15", "15 - 20"))
  ) |>
  left_join(harvest_summary, by = join_by(StandID)) |>
  ggplot(aes(TimeStep, Aboveground_Total_Live, fill = HarvestIntensity)) +
  facet_wrap("HarvestIntensity") +
