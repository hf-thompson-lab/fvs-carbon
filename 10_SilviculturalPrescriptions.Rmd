...
```{r trees_harvested_but_not_removed}
...
stopifnot(trees_harvested_but_not_removed |> nrow() == 0)
```

It is expected that FIA did not mark all removed trees as harvested.
```{r trees_removed_but_not_harvested}
trees_removed_but_not_harvested <- trees_removed_in_harvest |>
  anti_join(tree_harvested, by = join_by(STATECD, COUNTYCD, PLOT, SUBP, TREE))
trees_removed_but_not_harvested |> nrow()
```

All harvested trees should be on harvested plots.
```{r stopifnot_trees_removed_from_non_harvested_plots}
stopifnot(
...
```{r plot_harvested_without_trees_removed}
plot_harvested_without_trees_removed <- plot_harvested |>
  anti_join(
    trees_removed_in_harvest |>
      distinct(STATECD, COUNTYCD, PLOT),
    join_by(STATECD, COUNTYCD, PLOT)
  )
plot_harvested_without_trees_removed |> nrow()
```
...
join_cols <- plot_harvested_without_trees_removed |>
  names()
plot_harvested <- plot_harvested |>
  anti_join(plot_harvested_without_trees_removed, join_cols)
stand_stats <- stand_stats |>
  anti_join(plot_harvested_without_trees_removed, join_cols)
tree_harvested <- tree_harvested |>
  anti_join(plot_harvested_without_trees_removed, join_cols)
rm(join_cols)
```
