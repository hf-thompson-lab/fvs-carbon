...
run this block to pre-load them.

Regenerate this block with
```{sh, eval = FALSE}
fgrep '```{targets' 05_NRSNoManagement.Rmd | \
  sed -E -e 's/```{targets /tar_load(/' -e 's/,.*/)/' 
```

```{r load tar objects defined here, eval = FALSE}
tar_load(nrs_plots_grown)
tar_load(fvsne_states)
tar_load(nrs_plots_stats)
tar_load(nrs_grm_ingrowth_all)
tar_load(nrs_trees_ingrowth_all)
tar_load(nrs_grm_ingrowth)
tar_load(nrs_trees_ingrowth)
tar_load(nrs_trees_grown)
tar_load(nrs_trees_history)
```
...
ggplot(fvsne_states_map, aes(long, lat, group = group)) +
  ...
  ggtitle("FIA Grow-Only Plots using FVSne")
...
tree_growth_fia <- nrs_trees_history |>
  ...
  group_by(FIRST_CN) |>
  mutate(
    FRST_DIA = min(DIA, na.rm = TRUE),
    FRST_MEASYEAR = min(MEASYEAR, na.rm = TRUE)
  ) |>
  ungroup() |>
...
```{r tree_growth_with_fixed_spcd, eval = FALSE}
...
```{r ingrowth_stem_rate, eval = FALSE}
...
```{r plot-ingrowth_stem_rate, eval = FALSE}
...
tree_growth_ht_plot <- trees_for_ht_estimation |>
  group_by(STATECD, COUNTYCD, PLOT, SPCD) |>
  arrange(abs(DIA - 3)) |>
  filter(row_number() <= 9) |>
...
```{r plot tree_growth_ht origin, eval = FALSE}
tree_growth_ht |>
  mutate(
    HT_ORIGIN = case_when(
      !is.na(HT_PLOT) ~ "Plot",
  ...
  ggplot(aes(x = factor(1), fill = HT_ORIGIN)) +
  geom_bar(width = 1) + 
  ylab(NULL) +
  xlab(NULL) +
...
tree_growth_ht_whence <- trees_for_ht_estimation |>
...
> TODO nik: when density > 1000, try breaking it into multiple records, e.g.
> instead of
> Natural            0        RM    1500       100                  29         0
> try
> Natural            0        RM    1000       100                  29         0
> Natural            0        RM     500       100                  29         0
> CAVEAT:
> FVS says it has a hard limit of 3000

```{r regen_table, eval = FALSE}
...
```{r keywordfile_section, eval = FALSE}
...
FVS_Carbon_SRVY |>
  left_join(
    stands_surveyed |> select(STAND_ID, STATECD, COUNTYCD, PLOT),
    by = join_by(StandID == STAND_ID)
  ) |>
  mutate(StandID = sprintf('%02d%03d%05d', STATECD, COUNTYCD, PLOT)) |>
  select(StandID, Year, Aboveground_Total_Live) |>
  group_by(StandID) |>
  arrange(Year) |>
  mutate(
  ...
  full_join(
    expand.grid(
      StandID = unique(projected_vs_measured$STAND_ID),
      Year = all_years$Year
    ),
    by = join_by(StandID, Year)
  ) |>
  ...
  scale_fill_continuous(name = "Number of Plots", transform = "log10") +
  theme_bw() +
  ggtitle("FVS Observed Annual Carbon Flux for Grow-Only Plots") +
