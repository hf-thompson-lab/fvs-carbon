...
> -  Producing program, FVS
> -  Title; Northeastern
> -  MgmtId; NONE
> -  Partition
> -  File type
> So,
> FVS_Northeastern_NONE_part1.key
> FVS_Northeastern_NONE_part1.db
> etc.
...
"EndSQL\n",
"DSNOut\n",
"FVS_Northeastern_NONE_", Partition, ".db\n",
"Summary            2\n",
"Computdb           0         1\n",
"MisRpts            2\n",
"CarbReDB           2\n",
...
for (partition in 1:8) {
  filename <- paste0("FVS_Northeastern_NONE_", partition, ".key")
  write_lines(
    apply(
...
```{r}
projected_carbon_tmp <- FVS_Carbon |>
  # TODO nik: why do these stands get multiple, identical cases?
  filter(CaseID != '3dd9a3f0-57e9-4739-8fd7-58e45bbc3896') |>
  filter(CaseID != '1a3f3b16-7f96-4aa4-a415-0d79fd555acc') |>
  select(StandID, Year, Aboveground_Total_Live) |>
  rename(Projected_Carbon = Aboveground_Total_Live)

projected_ba_tmp <- FVS_Summary2_East |>
  filter(CaseID != '3dd9a3f0-57e9-4739-8fd7-58e45bbc3896') |>
  filter(CaseID != '1a3f3b16-7f96-4aa4-a415-0d79fd555acc') |>
  select(StandID, Year, BA) |>
  rename(Projected_BA = BA)
...
projected_vs_measured <- stand_stats |>
  ...
  rename(Measured_BA = BALIVE_METRIC) |>
  rename(Measured_Carbon = CARBON_METRIC) |>
  select(
    STAND_ID, Year, First_Year,
    STDAGE, `Forest Type`, `Forest Type Group`,
    Measured_BA, Measured_Carbon
  ) |>
  mutate(`Projection Years` = Year - First_Year) |>
  left_join(projected_tmp, by = join_by(STAND_ID, Year)) |>
  filter(!is.na(Projected_Carbon) & !is.na(Measured_Carbon))

remove(projected_carbon_tmp, projected_ba_tmp, projected_tmp, first_measyear_tmp)
```
...
ggplot(
  ...
  geom_point(
    size = 2,
    alpha = 0.5,
    mapping = aes(x = Measured_Carbon, y = Projected_Carbon, color = `Projection Years`)
  ) +
  geom_abline(intercept = 0, slope = 1) +
  facet_wrap(~`Forest Type Group`) +
  ggtitle("Grow-Only Stands\nFIA Measured vs. FVS Projected Carbon (Mg/ha)") +
