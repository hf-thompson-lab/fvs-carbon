...
matching_plot <- fia_plot |>
  semi_join(
    nk_to_fia |> select(STATECD, UNITCD, COUNTYCD, PLOT),
...
knitr::kable(
  matching_cond |>
    filter(STATECD == 33 & UNITCD == 2 & COUNTYCD == 3 & PLOT == 163)
)
```
...
comparable_carbon <- NoManagement_Carbon_ByPlot |>
  ...
  select(StandID, Year, Standing_Carbon)
knitr::kable(comparable_carbon)
```

```{r}

# Filter projections to just plots that were measured.
# Sum for each year.

Projected_Carbon <- NoManagement_Carbon_ByPlot |>
  semi_join()
  group_by(Year) |>
  summarize(
    Total_Carbon=mean(Total_Stand_Carbon),
