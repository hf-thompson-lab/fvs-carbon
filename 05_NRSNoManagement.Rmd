...
tar_load(nrs_trees_spcd)
tar_load(nrs_estab_rate)
tar_load(nrs_estab_height)
tar_load(nrs_plots_grow_partitions)
tar_load(nrs_plots_grow_partition)
tar_load(nrs_plots_grow_randseed)
tar_load(nrs_growonly_none)
tar_load(nrs_growonly_srvy)
```
...
```{r nrs_growonly_none_carbon, eval = FALSE}
nrs_growonly_none_carbon <- fvs_read_output(nrs_growonly_none, "FVS_Carbon")
```

```{r nrs_growonly_srvy_carbon, eval = FALSE}
nrs_growonly_srvy_carbon <- fvs_read_output(nrs_growonly_srvy, "FVS_Carbon")
```
...
FVS_Carbon_NONE <- nrs_growonly_none_carbon |>
  group_by(StandID, Year) |> # Combine results from different random seeds
  summarize(Aboveground_Total_Live = mean(Abovegroun_Total_Live))

FVS_Carbon_SRVY <- nrs_growonly_srvy_carbon
```
