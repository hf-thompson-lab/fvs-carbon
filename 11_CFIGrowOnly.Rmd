...
Without folding black oak into red oak, it selected the two projected
quantities, and two of the three species selectors, and LAT.

With folding black oak into red oak, the RMSE for the top 5 combinations
was all within 1%, so it looks like we can pick any convenient blend of
species, projected quantities, and ELEV and LAT.

## Hyperparameters

```{r optimize hyperparameters, eval = FALSE}
cfigro_bai_task$select(c(
  "Acer_rubrum", "ELEV", "LAT",
  "Projected_BA", "Projected_Tph", "Pinus_strobus"
))
...
> unlist(nrsres_bai_hp_task$result_x_domain)
   importance     max.depth min.node.size          mtry     num.trees 
"permutation"           "6"           "8"           "3"          "45" 

  

# Without folding red oak into black oak
> unlist(nrsres_bai_hp_task$result_x_domain)
   importance     max.depth min.node.size          mtry     num.trees 
   "impurity"          "16"          "11"           "2"          "68" 
> unlist(nrsres_bai_hp_task$result_x_domain)
