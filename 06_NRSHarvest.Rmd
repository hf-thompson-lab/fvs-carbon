...
```{sh, eval = FALSE}
fgrep '```{targets' 06_NRSHarvest.Rmd | \
...
```{r nrs_plot_prsc, cache=TRUE}
nrs_plots_prsc <- fia_plots_filtered(fiadb, filter = \(.data, con) {
  .data |>
    filter(INVYR >= 1999) |>
...
```{r nrs_plots_prsc_stats, tar_simple = TRUE}
nrs_plots_prsc_stats <- fia_plots_filtered(fiadb, nrs_plots_prsc, \(.data, con) {
...
```{r map_by_ecoregion}
plot_location_tmp <- nrs_plots_prsc |>
  left_join(
    nrs_plots_prsc_stats |> select(STATECD, COUNTYCD, PLOT, INVYR, FOREST_TYPE_GROUP),
